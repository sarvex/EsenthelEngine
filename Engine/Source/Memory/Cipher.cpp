...
// AES requires 16-byte alignment due to _mm_* operations
...
// CIPHER 1
/******************************************************************************/
         Cipher1::Cipher1() {setKey(null, 0);}
Cipher1& Cipher1::setKey(C Byte *key, Int key_size)
{
   if(key && key_size>0)
   {
       REP(256)_enc[i]=i;
...
Cipher1::Cipher1(Byte k00, Byte k01, Byte k02, Byte k03, Byte k04, Byte k05, Byte k06, Byte k07, Byte k08, Byte k09, Byte k0A, Byte k0B, Byte k0C, Byte k0D, Byte k0E, Byte k0F,
...
void Cipher1::setDec()
{
   REP(256)_dec[_enc[i]]=i;
}
/******************************************************************************/
void Cipher1::randomizeKey()
{
   UID uid[256/SIZE(UID)]; REPAO(uid).randomize(); // 'UID.randomize' uses a more secure version than 'Random'
   setKey(uid[0].b, SIZE(uid));
}
Bool Cipher1::saveKey(File &f)C {return  f.put(_enc, 256);}
Bool Cipher1::loadKey(File &f)  {Bool ok=f.get(_enc, 256); setDec(); wrapKey(); return ok;}
Bool Cipher1:: mixKey(File &f)  {Byte temp[256]; f>>temp; REPAO(temp)^=_enc[i]; setKey(temp, Elms(temp)); return f.ok();}
/******************************************************************************/
#pragma runtime_checks("", off)
void Cipher1::encrypt(Ptr dest, CPtr src, IntPtr size, Int offset)
{
   if(Byte *d=(Byte*)dest)
   {
      offset&=255;
      if(C Byte *s=(Byte*)src)
      {
         REPP(size/4) // doing 'VecB4' reads/writes made things slower, so process each 'Byte' separately, "for(; size>=4; size-=4)" was slower for size>=1024
         {
           ...
           *d++=_enc[(*s++)^_enc[offset+1]];
           *d++=_enc[(*s++)^_enc[offset+2]];
           *d++=_enc[(*s++)^_enc[offset+3]];
            offset+=4; offset&=255;
         }
         switch(size%4) // this is faster than REP(size%4), can't use "size&3" due to negative numbers
         {
            case 3: *d++=_enc[(*s++)^_enc[offset]];   offset++; // !! no break on purpose !!
            case 2: *d++=_enc[(*s++)^_enc[offset]];   offset++; // !! no break on purpose !!
            case 1: *d++=_enc[(*s++)^_enc[offset]]; //offset++; // !! no break on purpose !!
         }
      }else
      {
         REPP(size/4) // doing 'VecB4' reads/writes made things slower, so process each 'Byte' separately, "for(; size>=4; size-=4)" was slower for size>=1024
         {
           ...
           *d++=_enc[_enc[offset+1]];
           *d++=_enc[_enc[offset+2]];
           *d++=_enc[_enc[offset+3]];
            offset+=4; offset&=255;
         }
         switch(size%4) // this is faster than REP(size%4), can't use "size&3" due to negative numbers
         {
            case 3: *d++=_enc[_enc[offset]];   offset++; // !! no break on purpose !!
            case 2: *d++=_enc[_enc[offset]];   offset++; // !! no break on purpose !!
            case 1: *d++=_enc[_enc[offset]]; //offset++; // !! no break on purpose !!
         }
      }
   }
}
void Cipher1::decrypt(Ptr dest, CPtr src, IntPtr size, Int offset)
{
   if(Byte *d=(Byte*)dest)
   {
      offset&=255;
      if(C Byte *s=(Byte*)src)
      {
         REPP(size/4) // doing 'VecB4' reads/writes made things slower, so process each 'Byte' separately, "for(; size>=4; size-=4)" was slower for size>=1024
         {
           ...
           *d++=_dec[(*s++)]^_enc[offset+1];
           *d++=_dec[(*s++)]^_enc[offset+2];
           *d++=_dec[(*s++)]^_enc[offset+3];
            offset+=4; offset&=255;
         }
         switch(size%4) // this is faster than REP(size%4), can't use "size&3" due to negative numbers
         {
            case 3: *d++=_dec[(*s++)]^_enc[offset];   offset++; // !! no break on purpose !!
            case 2: *d++=_dec[(*s++)]^_enc[offset];   offset++; // !! no break on purpose !!
            case 1: *d++=_dec[(*s++)]^_enc[offset]; //offset++; // !! no break on purpose !!
         }
      }else
      {
         Byte dec0=_dec[0];
         REPP(size/4) // doing 'VecB4' reads/writes made things slower, so process each 'Byte' separately, "for(; size>=4; size-=4)" was slower for size>=1024
         {
           ...
           *d++=dec0^_enc[offset+1];
           *d++=dec0^_enc[offset+2];
           *d++=dec0^_enc[offset+3];
            offset+=4; offset&=255;
         }
         switch(size%4) // this is faster than REP(size%4), can't use "size&3" due to negative numbers
         {
            case 3: *d++=dec0^_enc[offset];   offset++; // !! no break on purpose !!
            case 2: *d++=dec0^_enc[offset];   offset++; // !! no break on purpose !!
            case 1: *d++=dec0^_enc[offset]; //offset++; // !! no break on purpose !!
         }
      }
   }
}
...
// CIPHER 2
/******************************************************************************/
         Cipher2::Cipher2() {setKey(null, 0);}
Cipher2& Cipher2::setKey(C Byte *key, Int key_size)
{
   if(key && key_size>0)
   {
      REPAO(_enc)=i;
...
Cipher2::Cipher2(Byte k00, Byte k01, Byte k02, Byte k03, Byte k04, Byte k05, Byte k06, Byte k07, Byte k08, Byte k09, Byte k0A, Byte k0B, Byte k0C, Byte k0D, Byte k0E, Byte k0F,
                 Byte k10, Byte k11, Byte k12, Byte k13, Byte k14, Byte k15, Byte k16, Byte k17, Byte k18, Byte k19, Byte k1A, Byte k1B, Byte k1C, Byte k1D, Byte k1E, Byte k1F,
                 Byte k20, Byte k21, Byte k22, Byte k23, Byte k24, Byte k25, Byte k26, Byte k27, Byte k28, Byte k29, Byte k2A, Byte k2B, Byte k2C, Byte k2D, Byte k2E, Byte k2F,
                 Byte k30, Byte k31, Byte k32, Byte k33, Byte k34, Byte k35, Byte k36, Byte k37, Byte k38, Byte k39, Byte k3A, Byte k3B, Byte k3C, Byte k3D, Byte k3E, Byte k3F,
                 Byte k40, Byte k41, Byte k42, Byte k43, Byte k44, Byte k45, Byte k46, Byte k47, Byte k48, Byte k49, Byte k4A, Byte k4B, Byte k4C, Byte k4D, Byte k4E, Byte k4F,
...
void Cipher2::setDec()
{
   REPA(_dec)_dec[_enc[i]]=i;
}
/******************************************************************************/
void Cipher2::randomizeKey()
{
   UID uid[256/SIZE(UID)]; REPAO(uid).randomize(); // 'UID.randomize' uses a more secure version than 'Random'
   setKey(uid[0].b, SIZE(uid));
}
Bool Cipher2::saveKey(File &f)C {return  f.put(_enc, 256);}
Bool Cipher2::loadKey(File &f)  {Bool ok=f.get(_enc, 256); setDec(); return ok;}
Bool Cipher2:: mixKey(File &f)  {Byte temp[256]; f>>temp; REPAO(temp)^=_enc[i]; setKey(temp, Elms(temp)); return f.ok();}
/******************************************************************************/
#pragma runtime_checks("", off)
void Cipher2::encrypt(Ptr dest, CPtr src, IntPtr size, Int offset)
{
   if(Byte *d=(Byte*)dest)
   {
      if(C Byte *s=(Byte*)src)
      {
         REPP(size/4) // doing 'VecB4' reads/writes made things slower, so process each 'Byte' separately, "for(; size>=4; size-=4)" was slower for size>=1024
         {
           *d++=_enc[_enc[(*s++)^(offset&0xFF)]^((offset>>8)&0xFF)]; offset++;
           *d++=_enc[_enc[(*s++)^(offset&0xFF)]^((offset>>8)&0xFF)]; offset++;
           *d++=_enc[_enc[(*s++)^(offset&0xFF)]^((offset>>8)&0xFF)]; offset++;
           *d++=_enc[_enc[(*s++)^(offset&0xFF)]^((offset>>8)&0xFF)]; offset++;
         }
         switch(size%4) // this is faster than REP(size%4), can't use "size&3" due to negative numbers
         {
            case 3: *d++=_enc[_enc[(*s++)^(offset&0xFF)]^((offset>>8)&0xFF)];   offset++; // !! no break on purpose !!
            case 2: *d++=_enc[_enc[(*s++)^(offset&0xFF)]^((offset>>8)&0xFF)];   offset++; // !! no break on purpose !!
            case 1: *d++=_enc[_enc[(*s++)^(offset&0xFF)]^((offset>>8)&0xFF)]; //offset++; // !! no break on purpose !!
         }
      }else
      {
         REPP(size/4) // doing 'VecB4' reads/writes made things slower, so process each 'Byte' separately, "for(; size>=4; size-=4)" was slower for size>=1024
         {
           *d++=_enc[_enc[offset&0xFF]^((offset>>8)&0xFF)]; offset++;
           *d++=_enc[_enc[offset&0xFF]^((offset>>8)&0xFF)]; offset++;
           *d++=_enc[_enc[offset&0xFF]^((offset>>8)&0xFF)]; offset++;
           *d++=_enc[_enc[offset&0xFF]^((offset>>8)&0xFF)]; offset++;
         }
         switch(size%4) // this is faster than REP(size%4), can't use "size&3" due to negative numbers
         {
            case 3: *d++=_enc[_enc[offset&0xFF]^((offset>>8)&0xFF)];   offset++; // !! no break on purpose !!
            case 2: *d++=_enc[_enc[offset&0xFF]^((offset>>8)&0xFF)];   offset++; // !! no break on purpose !!
            case 1: *d++=_enc[_enc[offset&0xFF]^((offset>>8)&0xFF)]; //offset++; // !! no break on purpose !!
         }
      }
   }
}
void Cipher2::decrypt(Ptr dest, CPtr src, IntPtr size, Int offset)
{
   if(Byte *d=(Byte*)dest)
   {
      if(C Byte *s=(Byte*)src)
      {
         REPP(size/4) // doing 'VecB4' reads/writes made things slower, so process each 'Byte' separately, "for(; size>=4; size-=4)" was slower for size>=1024
         {
           *d++=_dec[_dec[*s++]^((offset>>8)&0xFF)]^offset; offset++;
           *d++=_dec[_dec[*s++]^((offset>>8)&0xFF)]^offset; offset++;
           *d++=_dec[_dec[*s++]^((offset>>8)&0xFF)]^offset; offset++;
           *d++=_dec[_dec[*s++]^((offset>>8)&0xFF)]^offset; offset++;
         }
         switch(size%4) // this is faster than REP(size%4), can't use "size&3" due to negative numbers
         {
            case 3: *d++=_dec[_dec[*s++]^((offset>>8)&0xFF)]^offset;   offset++; // !! no break on purpose !!
            case 2: *d++=_dec[_dec[*s++]^((offset>>8)&0xFF)]^offset;   offset++; // !! no break on purpose !!
            case 1: *d++=_dec[_dec[*s++]^((offset>>8)&0xFF)]^offset; //offset++; // !! no break on purpose !!
         }
      }else
      {
         Byte dec0=_dec[0];
         REPP(size/4) // doing 'VecB4' reads/writes made things slower, so process each 'Byte' separately, "for(; size>=4; size-=4)" was slower for size>=1024
         {
           *d++=_dec[dec0^((offset>>8)&0xFF)]^offset; offset++;
           *d++=_dec[dec0^((offset>>8)&0xFF)]^offset; offset++;
           *d++=_dec[dec0^((offset>>8)&0xFF)]^offset; offset++;
           *d++=_dec[dec0^((offset>>8)&0xFF)]^offset; offset++;
         }
         switch(size%4) // this is faster than REP(size%4), can't use "size&3" due to negative numbers
         {
            case 3: *d++=_dec[dec0^((offset>>8)&0xFF)]^offset;   offset++; // !! no break on purpose !!
            case 2: *d++=_dec[dec0^((offset>>8)&0xFF)]^offset;   offset++; // !! no break on purpose !!
            case 1: *d++=_dec[dec0^((offset>>8)&0xFF)]^offset; //offset++; // !! no break on purpose !!
         }
      }
   }
}
...
// CIPHER 3
/******************************************************************************/
         Cipher3::Cipher3() {setKey(null, 0);}
Cipher3& Cipher3::setKey(C Byte *key, Int key_size)
{
   if(key && key_size>0)
   {
      REPAO(_enc)=i;
...
Cipher3::Cipher3(Byte k00, Byte k01, Byte k02, Byte k03, Byte k04, Byte k05, Byte k06, Byte k07, Byte k08, Byte k09, Byte k0A, Byte k0B, Byte k0C, Byte k0D, Byte k0E, Byte k0F,
                 Byte k10, Byte k11, Byte k12, Byte k13, Byte k14, Byte k15, Byte k16, Byte k17, Byte k18, Byte k19, Byte k1A, Byte k1B, Byte k1C, Byte k1D, Byte k1E, Byte k1F,
                 Byte k20, Byte k21, Byte k22, Byte k23, Byte k24, Byte k25, Byte k26, Byte k27, Byte k28, Byte k29, Byte k2A, Byte k2B, Byte k2C, Byte k2D, Byte k2E, Byte k2F,
                 Byte k30, Byte k31, Byte k32, Byte k33, Byte k34, Byte k35, Byte k36, Byte k37, Byte k38, Byte k39, Byte k3A, Byte k3B, Byte k3C, Byte k3D, Byte k3E, Byte k3F,
                 Byte k40, Byte k41, Byte k42, Byte k43, Byte k44, Byte k45, Byte k46, Byte k47, Byte k48, Byte k49, Byte k4A, Byte k4B, Byte k4C, Byte k4D, Byte k4E, Byte k4F,
...
/******************************************************************************/
void Cipher3::randomizeKey()
{
   UID uid[256/SIZE(UID)]; REPAO(uid).randomize(); // 'UID.randomize' uses a more secure version than 'Random'
   setKey(uid[0].b, SIZE(uid));
}
Bool Cipher3::saveKey(File &f)C {return  f.put(_enc, 256);}
Bool Cipher3::loadKey(File &f)  {Bool ok=f.get(_enc, 256); setDec(); return ok;}
Bool Cipher3:: mixKey(File &f)  {Byte temp[256]; f>>temp; REPAO(temp)^=_enc[i]; setKey(temp, Elms(temp)); return f.ok();}
/******************************************************************************/
#pragma runtime_checks("", off)
void Cipher3::encrypt(Ptr dest, CPtr src, IntPtr size, Int offset)
{
   if(Byte *d=(Byte*)dest)
   {
      if(C Byte *s=(Byte*)src)
      {
         REPP(size/4) // doing 'VecB4' reads/writes made things slower, so process each 'Byte' separately, "for(; size>=4; size-=4)" was slower for size>=1024
         {
           *d++=_enc[_enc[_enc[(*s++)^(offset&0xFF)]^((offset>>8)&0xFF)]^((offset>>16)&0xFF)]; offset++;
           *d++=_enc[_enc[_enc[(*s++)^(offset&0xFF)]^((offset>>8)&0xFF)]^((offset>>16)&0xFF)]; offset++;
           *d++=_enc[_enc[_enc[(*s++)^(offset&0xFF)]^((offset>>8)&0xFF)]^((offset>>16)&0xFF)]; offset++;
           *d++=_enc[_enc[_enc[(*s++)^(offset&0xFF)]^((offset>>8)&0xFF)]^((offset>>16)&0xFF)]; offset++;
         }
         switch(size%4) // this is faster than REP(size%4), can't use "size&3" due to negative numbers
         {
            case 3: *d++=_enc[_enc[_enc[(*s++)^(offset&0xFF)]^((offset>>8)&0xFF)]^((offset>>16)&0xFF)];   offset++; // !! no break on purpose !!
            case 2: *d++=_enc[_enc[_enc[(*s++)^(offset&0xFF)]^((offset>>8)&0xFF)]^((offset>>16)&0xFF)];   offset++; // !! no break on purpose !!
            case 1: *d++=_enc[_enc[_enc[(*s++)^(offset&0xFF)]^((offset>>8)&0xFF)]^((offset>>16)&0xFF)]; //offset++; // !! no break on purpose !!
         }
      }else
      {
         REPP(size/4) // doing 'VecB4' reads/writes made things slower, so process each 'Byte' separately, "for(; size>=4; size-=4)" was slower for size>=1024
         {
           *d++=_enc[_enc[_enc[offset&0xFF]^((offset>>8)&0xFF)]^((offset>>16)&0xFF)]; offset++;
           *d++=_enc[_enc[_enc[offset&0xFF]^((offset>>8)&0xFF)]^((offset>>16)&0xFF)]; offset++;
           *d++=_enc[_enc[_enc[offset&0xFF]^((offset>>8)&0xFF)]^((offset>>16)&0xFF)]; offset++;
           *d++=_enc[_enc[_enc[offset&0xFF]^((offset>>8)&0xFF)]^((offset>>16)&0xFF)]; offset++;
         }
         switch(size%4) // this is faster than REP(size%4), can't use "size&3" due to negative numbers
         {
            case 3: *d++=_enc[_enc[_enc[offset&0xFF]^((offset>>8)&0xFF)]^((offset>>16)&0xFF)];   offset++; // !! no break on purpose !!
            case 2: *d++=_enc[_enc[_enc[offset&0xFF]^((offset>>8)&0xFF)]^((offset>>16)&0xFF)];   offset++; // !! no break on purpose !!
            case 1: *d++=_enc[_enc[_enc[offset&0xFF]^((offset>>8)&0xFF)]^((offset>>16)&0xFF)]; //offset++; // !! no break on purpose !!
         }
      }
   }
}
void Cipher3::decrypt(Ptr dest, CPtr src, IntPtr size, Int offset)
{
   if(Byte *d=(Byte*)dest)
   {
      if(C Byte *s=(Byte*)src)
      {
         REPP(size/4) // doing 'VecB4' reads/writes made things slower, so process each 'Byte' separately, "for(; size>=4; size-=4)" was slower for size>=1024
         {
           *d++=_dec[_dec[_dec[*s++]^((offset>>16)&0xFF)]^((offset>>8)&0xFF)]^offset; offset++;
           *d++=_dec[_dec[_dec[*s++]^((offset>>16)&0xFF)]^((offset>>8)&0xFF)]^offset; offset++;
           *d++=_dec[_dec[_dec[*s++]^((offset>>16)&0xFF)]^((offset>>8)&0xFF)]^offset; offset++;
           *d++=_dec[_dec[_dec[*s++]^((offset>>16)&0xFF)]^((offset>>8)&0xFF)]^offset; offset++;
         }
         switch(size%4) // this is faster than REP(size%4), can't use "size&3" due to negative numbers
         {
            case 3: *d++=_dec[_dec[_dec[*s++]^((offset>>16)&0xFF)]^((offset>>8)&0xFF)]^offset;   offset++; // !! no break on purpose !!
            case 2: *d++=_dec[_dec[_dec[*s++]^((offset>>16)&0xFF)]^((offset>>8)&0xFF)]^offset;   offset++; // !! no break on purpose !!
            case 1: *d++=_dec[_dec[_dec[*s++]^((offset>>16)&0xFF)]^((offset>>8)&0xFF)]^offset; //offset++; // !! no break on purpose !!
         }
      }else
      {
         Byte dec0=_dec[0];
         REPP(size/4) // doing 'VecB4' reads/writes made things slower, so process each 'Byte' separately, "for(; size>=4; size-=4)" was slower for size>=1024
         {
           *d++=_dec[_dec[dec0^((offset>>16)&0xFF)]^((offset>>8)&0xFF)]^offset; offset++;
           *d++=_dec[_dec[dec0^((offset>>16)&0xFF)]^((offset>>8)&0xFF)]^offset; offset++;
           *d++=_dec[_dec[dec0^((offset>>16)&0xFF)]^((offset>>8)&0xFF)]^offset; offset++;
           *d++=_dec[_dec[dec0^((offset>>16)&0xFF)]^((offset>>8)&0xFF)]^offset; offset++;
         }
         switch(size%4) // this is faster than REP(size%4), can't use "size&3" due to negative numbers
         {
            case 3: *d++=_dec[_dec[dec0^((offset>>16)&0xFF)]^((offset>>8)&0xFF)]^offset;   offset++; // !! no break on purpose !!
            case 2: *d++=_dec[_dec[dec0^((offset>>16)&0xFF)]^((offset>>8)&0xFF)]^offset;   offset++; // !! no break on purpose !!
            case 1: *d++=_dec[_dec[dec0^((offset>>16)&0xFF)]^((offset>>8)&0xFF)]^offset; //offset++; // !! no break on purpose !!
         }
      }
   }
}
...
{
   0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000, 0x20000000, 0x40000000, 0x80000000, 0x1B000000, 0x36000000, // for 128-bit blocks, Rijndael never uses more than 10 rcon values
};
static const U32 rcLE[]=
{
	0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36, // for 128-bit blocks, Rijndael never uses more than 10 rcon values
};
AES::AES() {Zero(T);}
AES_ATTR Bool AES::create(CPtr key_data, Int key_size)
{
   if(C Byte *key=(Byte*)key_data)
   {
   #if AES_CPU
      if(Cpu.flag()&CPU_AES)
      {
         if(key_size!=16 && key_size!=24 && key_size!=32)return false;
        _rounds=6+key_size/4;

         const U32 *rc=rcLE;
               U32 *rk=_encrypt_key;

         memcpy(rk, key, key_size);
         __m128i temp=_mm_loadu_si128((__m128i*)(key+key_size-16));

         for(;;)
         {
            rk[key_size/4  ] = rk[0] ^ _mm_extract_epi32(_mm_aeskeygenassist_si128(temp, 0), 3) ^ *(rc++);
            rk[key_size/4+1] = rk[1] ^ rk[key_size/4  ];
            rk[key_size/4+2] = rk[2] ^ rk[key_size/4+1];
            rk[key_size/4+3] = rk[3] ^ rk[key_size/4+2];

            if(rk + key_size/4 + 4 >= _encrypt_key+ELMS(_encrypt_key))break;
         ...
         Int i, j; for(i=4, j=4*_rounds-4; i<j; i+=4, j-=4)
         {
      ...
      // first, start with an encryption schedule invert the order of the round keys
      for(Int i=0, j=4*_rounds; j>=0; i+=4, j-=4)
      {
        _decrypt_key[j  ]=_encrypt_key[i  ];
        _decrypt_key[j+1]=_encrypt_key[i+1];
        _decrypt_key[j+2]=_encrypt_key[i+2];
        _decrypt_key[j+3]=_encrypt_key[i+3];
      }
...
AES_ATTR void AES::decrypt16(Ptr dest, CPtr src)C
{
   ...
   s0 = ((U32)Td4[(t0 >> 24)] << 24) ^ ((U32)Td4[(t3 >> 16) & 0xff] << 16) ^ ((U32)Td4[(t2 >> 8) & 0xff] << 8) ^ ((U32)Td4[(t1) & 0xff]) ^ rk[0]; PUTU32(out   , s0);
   s1 = ((U32)Td4[(t1 >> 24)] << 24) ^ ((U32)Td4[(t0 >> 16) & 0xff] << 16) ^ ((U32)Td4[(t3 >> 8) & 0xff] << 8) ^ ((U32)Td4[(t2) & 0xff]) ^ rk[1]; PUTU32(out+ 4, s1);
   s2 = ((U32)Td4[(t2 >> 24)] << 24) ^ ((U32)Td4[(t1 >> 16) & 0xff] << 16) ^ ((U32)Td4[(t0 >> 8) & 0xff] << 8) ^ ((U32)Td4[(t3) & 0xff]) ^ rk[2]; PUTU32(out+ 8, s2);
   s3 = ((U32)Td4[(t3 >> 24)] << 24) ^ ((U32)Td4[(t2 >> 16) & 0xff] << 16) ^ ((U32)Td4[(t1 >> 8) & 0xff] << 8) ^ ((U32)Td4[(t0) & 0xff]) ^ rk[3]; PUTU32(out+12, s3);
}
#undef SWAP
#undef GETU32
#undef PUTU32
