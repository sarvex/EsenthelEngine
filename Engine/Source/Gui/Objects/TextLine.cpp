...
void TextLine::update(C GuiPC &gpc)
{
   ...
   if(   gpc_this.enabled)
   {
    ...
    C Vec2   *touch_pos  =null;
      ...
      if(Gui.kb()==this)REPA(Touches){Touch &touch=Touches[i]; if(touch.guiObj()==this && (touch._state&(BS_ON|BS_PUSHED))){touch_pos=&touch.pos(); touch_state=touch._state; touch.disableScroll(); break;}} // check touches only if we already have keyboard focus, so without focus we don't select but instead can scroll
