...
C MeshLod& Mesh::getDrawLod(C Matrix3 &matrix)C
{
   if(_lods.elms())
   {
...
Int Mesh::getDrawLodI(C Matrix3 &matrix)C
{
   if(_lods.elms())
   {
      Flt dist2=GetLodDist2(lod_center, matrix);
...
Mesh& Mesh::tesselate  (Flt weld_pos_eps) {REP(lods())lod(i).tesselate  (weld_pos_eps); return T;}
...
Bool  Mesh::weldCoplanarFaces    (Flt  cos_face, Flt cos_vtx, Bool safe, Flt max_face_length) {Bool changed=false; REP(lods())changed|=lod(i).weldCoplanarFaces    (cos_face, cos_vtx, safe, max_face_length);                                 return changed;}
