...
MeshBase& MeshBase::add(C MeshBase &src, MESH_FLAG flag_and)
{
   if(!src.is())return T;
 C MeshBase *meshes[]={this, &src};
   return create(meshes, Elms(meshes), flag_and);
}
MeshBase& MeshBase::add(C MeshRender &src, MESH_FLAG flag_and) {MeshBase mb(src, flag_and); return add(mb);}
MeshBase& MeshBase::add(C MeshPart   &src, MESH_FLAG flag_and) {MeshBase mb(src, flag_and); return add(mb);}
/******************************************************************************
