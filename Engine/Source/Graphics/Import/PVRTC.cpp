...
namespace PVRTC{
/***********************************************************/
typedef U8  PVRTuint8;
typedef I32 PVRTint32;
typedef U32 PVRTuint32;
/***********************************************************
...
static PVRTuint32 TwiddleUV(PVRTuint32 XSize, PVRTuint32 YSize, PVRTuint32 XPos, PVRTuint32 YPos)
{
	...
	DEBUG_ASSERT(YPos < YSize, "TwiddleUV");
	DEBUG_ASSERT(XPos < XSize, "TwiddleUV");
	DEBUG_ASSERT(isPowerOf2(YSize), "TwiddleUV");
	DEBUG_ASSERT(isPowerOf2(XSize), "TwiddleUV");
   ...
   if(dest.size3()==src.size3())
   {
      ...
      REPD(mip, Min(src.mipMaps(), dest.mipMaps(), max_mip_maps))
      {
         ...
         REPD(face, dest.faces()) // use 'dest.faces' (not 'target.faces' because it may be 1 for 'temp', and not 'src.faces' because we need to write all to dest)
         {
            ...
            if(!write_to_dest && !dest.injectMipMap(temp, mip, (DIR_ENUM)face, FILTER_NO_STRETCH, IC_CLAMP))return false;
         }
      }
      return true;
   }
