...
struct DDS_HEADER
{
   ...
   IMAGE_TYPE load(File &f)
   {
      ...
      if(IMAGE_TYPE type=pf.type())
      {
         ...
         {
            case IMAGE_BC1: case IMAGE_BC1_SRGB: case IMAGE_BC4: case IMAGE_BC4_SIGN:                                                                                           PitchOrLinearSize=DivCeil4(Width)* 8; break;
            case IMAGE_BC2: case IMAGE_BC2_SRGB: case IMAGE_BC3: case IMAGE_BC3_SRGB: case IMAGE_BC5: case IMAGE_BC5_SIGN: case IMAGE_BC6: case IMAGE_BC7: case IMAGE_BC7_SRGB: PitchOrLinearSize=DivCeil4(Width)*16; break;
...
Bool Image::ImportDDS(File &f, Int type, Int mode, Int mip_maps)
{
   ...
   if(f.getUInt()==CC4('D','D','S',' '))
   {
      ...
      {
         if(type    <=0)type    =file_type; if(type>=IMAGE_TYPES || type==IMAGE_BC1_SRGB)type=IMAGE_R8G8B8A8_SRGB; // don't return types not supported officially, also convert BC1 because it may contain alpha however engine assumes it doesn't #BC1RGB
