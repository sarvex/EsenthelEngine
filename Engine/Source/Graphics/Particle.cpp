...
Bool DrawParticleBegin(C Image &image, Byte glow, Bool motion_affects_alpha)
{
   ...
   switch(Renderer())
   {
      ...
      case RM_BLEND   : shader=Sh.Particle[false][soft][0][motion_affects_alpha]; ColorFunc=ColorAlpha; D.alpha(Renderer.fastCombine() ? ALPHA_BLEND : ALPHA_RENDER_BLEND_FACTOR); D.alphaFactor(Color(0, 0, 0, glow)); MaterialClear(); if(glow)Renderer._has|=HAS_GLOW; break; // 'MaterialClear' must be called when changing 'D.alphaFactor'
      case RM_PALETTE :
...
Bool DrawAnimatedParticleBegin(C Image &image, Byte glow, Bool motion_affects_alpha, Int x_frames, Int y_frames)
{
   ...
   {
      default         : return false;
      case RM_BLEND   : shader=Sh.Particle[false][soft][1+D.particlesSmoothAnim()][motion_affects_alpha]; ColorFunc=ColorAlpha; D.alpha(Renderer.fastCombine() ? ALPHA_BLEND : ALPHA_RENDER_BLEND_FACTOR); D.alphaFactor(Color(0, 0, 0, glow)); MaterialClear(); if(glow)Renderer._has|=HAS_GLOW; break; // 'MaterialClear' must be called when changing 'D.alphaFactor'
      case RM_PALETTE :
...
{
   File f; if(f.write(name)){if(save(f, include_particles, _GetPath(name)) && f.flush())return true; f.del(); FDelFile(name);}
   return false;
}
/******************************************************************************/
...
void RawParticles::draw()C
{
   if(image && _particles && Renderer()==renderMode())
   {
      ...
      {
         default         : return;
         case RM_BLEND   : shader=Sh.Particle[false][soft][0][motion_affects_alpha]; D.alpha(Renderer.fastCombine() ? ALPHA_BLEND : ALPHA_RENDER_BLEND_FACTOR); D.alphaFactor(Color(0, 0, 0, glow)); MaterialClear(); if(glow)Renderer._has|=HAS_GLOW; break; // 'MaterialClear' must be called when changing 'D.alphaFactor'
         case RM_PALETTE :
