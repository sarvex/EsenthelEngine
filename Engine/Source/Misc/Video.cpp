...
Bool Video::frameToImage(Int w, Int h, Int w_uv, Int h_uv, CPtr lum_data, CPtr u_data, CPtr v_data, Int lum_pitch, Int u_pitch, Int v_pitch)
{
   if(_mode==ALPHA)
   {
      if(!_lum.create2D(w, h, IMAGE_R8, 1, false))return false; _lum.setFaceData(lum_data, lum_pitch);
      return true;
   }else
   {
      if(!_lum.create2D(w   , h   , IMAGE_R8, 1, false))return false; _lum.setFaceData(lum_data, lum_pitch);
      if(!_u  .create2D(w_uv, h_uv, IMAGE_R8, 1, false))return false; _u  .setFaceData(  u_data,   u_pitch);
      if(!_v  .create2D(w_uv, h_uv, IMAGE_R8, 1, false))return false; _v  .setFaceData(  v_data,   v_pitch);
      return   createTex();
   }
}
Bool Video::frameToImage(Int w, Int h, Int w_uv, Int h_uv, CPtr lum_data, CPtr uv_data, Int lum_pitch, Int uv_pitch)
{
   ...
   {
      if(!_lum.create2D(w   , h   , IMAGE_R8  , 1, false))return false; _lum.setFaceData(lum_data, lum_pitch);
      if(!_u  .create2D(w_uv, h_uv, IMAGE_R8G8, 1, false))return false; _u  .setFaceData( uv_data,  uv_pitch);
          _v  .del();
      return   createTex();
   }
}
