...
struct EditorInterface
{
   ...
   // following methods operate on currently opened project
      ...
      Bool       reloadResult(C CMemPtr<UID> &elms, MemPtr< IDParam<RELOAD_RESULT> > results); // get element reload result for each element, you can call this after calling 'reloadElms' with 'remember_result' set to true
      Bool forgetReloadResult(C CMemPtr<UID> &elms                                          ); // if you've called 'reloadElms' with 'remember_result' set to true then the Editor will remember reload result for specified elements, with this function you can forget those results
      ...
      Bool modifyObject(C UID &elm_id, C CMemPtr<ObjChange> &changes                  ); // modify object data   of 'elm_id' ELM_OBJ ELM_OBJ_CLASS elements in the project according to specified list of 'changes', false on fail
