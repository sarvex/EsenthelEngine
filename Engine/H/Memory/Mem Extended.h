...
T1(TYPE) struct Memx : _Memx // Block Based Extended Container
{
 ...
 C TYPE& last      (     )C; // get last  valid    element
   ...
   Memx& sort(CPtr user, Int compare(C TYPE &a, C TYPE &b, CPtr user)        ); // sort elements with custom comparing function and 'user' parameter (sorting modifies only "valid indexes"), this method does not change the memory address of any of the elements
