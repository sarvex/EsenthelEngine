...
T1(TYPE) struct Meml : _Meml // List Based Container
{
 ...
 C TYPE& operator[](MemlNode *node)C; // get      MemlNode data, accessing element out of range is an invalid operation and may cause undefined behavior
   ...
   Meml& sort(CPtr user, Int compare(C TYPE &a, C TYPE &b, CPtr user)        ); // sort elements with custom comparing function and 'user' parameter, method does not change the memory address of any of the elements
