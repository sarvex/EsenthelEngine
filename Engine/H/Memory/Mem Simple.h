...
T1(const_mem_addr TYPE) struct Mems // Simple Continuous Memory Based Container
{
 ...
 C TYPE& last      (     )C; // get    last  element
   ...
   T1(VALUE)   Bool  binarySearchFirst(C VALUE &value, Int &index, Int compare(C TYPE &a, C VALUE &b)=Compare)C; // search sorted container for presence of 'value' and return if it was found in the container, 'index'=if the function returned true then this index points to the location where the 'value' is located in the container, if the function returned false then it means that 'value' was not found in the container however the 'index' points to the place where it should be added in the container while preserving sorted data, 'index' will always be in range (0..elms) inclusive, if there are multiple elements with same value, then the first one will be returned
