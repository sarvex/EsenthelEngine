...
Vec  SphereTerrainPosToDir        (DIR_ENUM cube_face, C Vec2 &xy           ); // convert spherical terrain    position    to vector direction, 'cube_face'=terrain cube face, 'xy' =terrain position          (-1..1   ) on plane Z=1                , returned vector is not normalized, however it's on a cube with radius=1 ("Abs(dir).max()=1")
