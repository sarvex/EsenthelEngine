...
Flt Dist2(C Edge &edge , C OBox   &obox ); // squared distance between edge  and a  box
...
Bool CutsLineBox(C Vec &line_pos, C Vec &line_dir, C Box    &box); // if infinite straight line cuts a  box   , 'line_dir'=line direction (doesn't have to be normalized)
Bool CutsLineBox(C Vec &line_pos, C Vec &line_dir, C Extent &ext); // if infinite straight line cuts an extent, 'line_dir'=line direction (doesn't have to be normalized)
