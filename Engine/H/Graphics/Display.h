...
struct DisplayClass : DisplayState, DisplayDraw // Display Control
{
 ...
 C Str8&           deviceName     ()C {return _device_name   ;} // get GPU device name
   ...
   DisplayClass& bumpMode         (BUMP_MODE        mode        );   BUMP_MODE        bumpMode          ()C {return _bump_mode        ;} // set/get Bump Mapping Mode                      (BUMP_MODE                          , default=      BUMP_RELIEF (BUMP_NORMAL     for Mobile)), using this method during application runtime requires setting 'D.set_shader' (please check comments on 'D.set_shader' for more info), the change is NOT instant, avoid calling real-time
   ...
   DisplayClass& highPrecPalRT    (Bool             on          );   Bool             highPrecPalRT     ()C {return _hp_pal_rt        ;} // set/get Palette   Render Target High Precision (true/false                         , default=            false                             ), enabling high precision render targets gives higher quality graphics at the cost of slower performance and more memory usage, high precision means that Render Targets are created using a different format, that is more precise but also uses more memory and bandwidth, Palette   Render Target is used for storing particles in RM_PALETTE                       , the change is NOT instant, avoid calling real-time
   ...
   DisplayClass& texDetailLOD     (Bool             on          );   Bool             texDetailLOD      ()C {return _tex_detail_lod   ;} // set/get Detail Textures on all Mesh LODs       (true/false                         , default=            false                             ), determines usage of additional Detail Textures in Materials, using this method during application runtime requires setting 'D.set_shader' (please check comments on 'D.set_shader' for more info), the change is NOT instant, avoid calling real-time, if this option is enabled then all LODs in a Mesh will use detail (if available), if disabled then only first LOD will use it
...
#endif
#endif
   DisplayClass& ambientMode    (AMBIENT_MODE mode      );   AMBIENT_MODE ambientMode    ()C {return _amb_mode         ;} // set/get Ambient Mode               (AMBIENT_MODE         , default=AMBIENT_LOW), the change is instant, you can call it real-time
