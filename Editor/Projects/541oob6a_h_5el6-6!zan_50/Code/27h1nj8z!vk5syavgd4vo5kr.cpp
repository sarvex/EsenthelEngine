...
bool Update()
{
   ...
   // set animations
   {   
      skel.updateBegin().clear(); // clear animated skeleton
...
void Render()
{
   switch(Renderer())
   {
      case RM_PREPARE:
      {
         chr->mesh()->draw(skel); // get mesh from cache and render it with animated skeleton
...
void Draw()
{
   ...
   if(Kb.ctrl()) // when control pressed
   {
      ...
      skel.draw(RED); // draw animated skeleton
   }
