...
class MtrlBrushClass : Window
{
   ...
   UID         scale_material_id=UIDZero;

   MtrlBrushClass() {REPAO(mtrl_id).zero();}

   static void PushedScale(MtrlBrushClass &mb) {SetHmOp(mb.scale() ? OP_HM_MTRL_SCALE : OP_HM_NONE);}
   static void PushedHole (MtrlBrushClass &mb) {SetHmOp(mb.hole () ? OP_HM_MTRL_HOLE  : OP_HM_NONE);}
   static void PushedMax1 (MtrlBrushClass &mb) {SetHmOp(mb.max1 () ? OP_HM_MTRL_MAX_1 : OP_HM_NONE);}
   static void PushedMax2 (MtrlBrushClass &mb) {SetHmOp(mb.max2 () ? OP_HM_MTRL_MAX_2 : OP_HM_NONE);}
   ...
   MtrlBrushClass& create()
   {
      ...
      Rect_LD rect(0, -WorldEdit.rect().h(), 1.585-skip*(s+e), s+b*2);
   #else
      int    skip=0;
      Rect_D rect(WorldEdit.rect().w()/2-0.16, -WorldEdit.rect().h(), 1.585, s+b*2);
   #endif
      ...
      T+=scale.create(Rect_L(x, -clientHeight()*1/4, 0.11, 0.04), MLTC(u"Scale", PL, u"Skaluj", DE, u"Skaliere", RU, u"Масштаб", PO, u"Escala", CN, u"缩放")).func(PushedScale, T).focusable(false).desc(MLTC(
