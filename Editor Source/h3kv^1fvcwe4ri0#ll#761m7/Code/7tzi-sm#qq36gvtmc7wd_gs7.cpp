...
class ObjPos : Region
{
   ...
   static void Cancel (ObjPos &op) {op.reset(); WorldEdit.obj_op.set(WorldView.OP_SEL);}
   ...
   ObjPos& create(C Vec2 &up)
   {
      ...
      super.create(Rect_U(up, 0.255, 0.22)).skin(&TransparentSkin, false).hide(); kb_lit=false;
      props.New().create("X", MemberDesc(DATA_REAL));
      props.New().create("Y", MemberDesc(DATA_REAL));
      props.New().create("Z", MemberDesc(DATA_REAL));
      flt x=AddProperties(props, T, Vec2(0.01, -0.01), 0.043, 0.20, &ts).centerX();
      REPAO(props).textline.func(Changed, T);

      T+=cancel.create(Rect_C(x, -0.18, 0.2, 0.05), "Cancel").func(Cancel, T);
      return T;
   }
   void apply()
   {
      ...
      if(gpc.visible && visible())
      {
         ...
         REPA(props)if(props[i].button()) // if at least one property is being edited then adjust scale of editing
         {
            ...
            if(!WorldEdit.v4.perspective() || !WorldEdit.v4.fpp())speed=CamMoveScale(WorldEdit.v4.perspective());else
            {
               ...
               speed=Dist(center, ActiveCam.matrix.pos)*0.75;
            }
            speed*=0.5*(Kb.shift() ? 0.1 : 1.0);
   ...
   void reset(int skip=-1)
   {
      ...
      FREPA(Selection)
      {
       C Matrix3 &m=Selection[i].matrix; Vec s=m.scale();
         if(!i)scale=s;else
         {
            multi_val[0]|=!Equal(scale.x, s.x, 0.001);
            multi_val[1]|=!Equal(scale.y, s.y, 0.001);
            multi_val[2]|=!Equal(scale.z, s.z, 0.001);
         }
      }
      scale_all=scale.max();
      multi_val[3]=(multi_val[0] || multi_val[1] || multi_val[2] || !Equal(scale.x, scale.y, 0.001) || !Equal(scale.x, scale.z, 0.001));
      if(props.elms()>=4)
      {
...
class ObjRot : Region
{
   ...
   class Rot
   {
      ...
      operator Matrix3() {Matrix3 m; m.setRotateZ(angles.z).rotateXY(angles.x, angles.y); m.x*=scale.x; m.y*=scale.y; m.z*=scale.z; return m;}
       Rot(C Matrix3 &m) {angles=Angles(m); scale=m.scale();}
   }
   ...
   {
      ts.reset().size=0.04; ts.align.set(1, 0);
      super.create(Rect_U(up, 0.255, 0.147)).skin(&TransparentSkin, false).hide(); kb_lit=false;
      ...
      AddProperties(props, T, Vec2(0.01, -0.01), 0.043, 0.20, &ts); REPAO(props).autoData(this).mouseEditSpeed(20);
      return T;
   }
   void reset(int skip=-1)
   {
...
class ObjGrid : Region
{
   ...
   ObjGrid& create()
   {
      super .create(Rect_LU(0, 0, 0.315, 0.125)).skin(&TransparentSkin, false); kb_lit=false;
      T+=bxz.create(Rect_L(0.01, -0.035, 0.17, 0.045), "Horizontal").focusable(false).desc("Enable Horizontal Grid Aligning"); bxz.mode=BUTTON_TOGGLE; bxz.set(true);
      T+=by .create(Rect_L(0.01, -0.090, 0.17, 0.045), "Vertical"  ).focusable(false).desc("Enable Vertical Grid Aligning"  ); by .mode=BUTTON_TOGGLE;
      T+= xz.create(Rect_L(0.19, -0.035, 0.11, 0.045), "1").desc(MLTC(u"Grid size (in meters)", PL, u"Rozmiar Siatki (w metrach)", DE, u"Gittergröße (in Meter)", RU, u"Размер сетки (в метрах)", PO, u"Tamanho da grade (em metros)"));
      T+= y .create(Rect_L(0.19, -0.090, 0.11, 0.045), "1").desc(MLTC(u"Grid size (in meters)", PL, u"Rozmiar Siatki (w metrach)", DE, u"Gittergröße (in Meter)", RU, u"Размер сетки (в метрах)", PO, u"Tamanho da grade (em metros)"));
      return T;
   }
   virtual void update(C GuiPC &gpc)override
   {
...
class WaypointPos : Region
{
   ...
   {
      ts.reset().size=0.04; ts.align.set(1, 0);
      super.create(Rect_U(up, 0.255, 0.15)).skin(&TransparentSkin, false).hide(); kb_lit=false;
      props.New().create("X", MemberDesc(DATA_REAL));
      props.New().create("Y", MemberDesc(DATA_REAL));
      props.New().create("Z", MemberDesc(DATA_REAL));
      AddProperties(props, T, Vec2(0.01, -0.01), 0.043, 0.20, &ts);
      REPAO(props).textline.func(Changed, T);
      return T;
   }
   void toGui()
   ...
   virtual void update(C GuiPC &gpc)override
   {
      ...
      if(gpc.visible && visible())
      {
         ...
         REPA(props)if(props[i].button()) // if at least one property is being edited then adjust scale of editing
         {
            ...
            if(WorldEdit.sel_waypoint)
               if(EditWaypointPoint *point=WorldEdit.sel_waypoint.find(WorldEdit.sel_waypoint_point))
                  speed=Dist(point.pos, ActiveCam.matrix.pos)*0.75;
            if(speed>0)
            {
               speed*=0.5*(Kb.shift() ? 0.1 : 1.0);
               REPAO(props).mouseEditSpeed(speed);
...
void WorldView.editObj()
{
   if(Selection.elms())
   {
      ...
      {
         obj_pos.apply();
         flt mul=ScaleFactor(Ms.wheel()*0.1*(Kb.shift() ? 0.2 : 1.0));
      ...
      if(  view_on && op!=OP_INS) // edit objects matrix
      {
         ...
         flt  multiplier =((Kb.shift() && !Kb.ctrlCmd()) ? 0.1 : 1.0);

         // setup operating values
         Vec center=0; if(Selection.elms()){REPA(Selection)center+=Selection[i].center(); center/=Selection.elms();}
         if(!v4.perspective() || !v4.fpp())obj_edit_speed=CamMoveScale(v4.perspective());else if(first_push)obj_edit_speed=Dist(center, ActiveCam.matrix.pos)*0.75; flt mul=obj_edit_speed*multiplier; // in FPP mode automatically detect the speed according to average object distance to camera, but only at the moment of first button push
               ...
               }else
               {
                  ...
                  switch(op)
                  {
                     ...
                     case OP_ROT_Y:
                     {
                        ...
                        {
                           Vec axis, nrm=obj.area.hmNormalAvg(matrix, obj.mesh ? obj.mesh->ext : Extent(0.5));
                  ...
                  if(matrix.pos!=obj.matrix.pos)
                  {
                     ...
                     if(align_nrm && obj.area)
                     {
                        Vec axis, nrm=obj.area.hmNormalAvg(obj.matrix, obj.mesh ? obj.mesh->ext : Extent(0.5)); // get the old normal
                        ...
                        nrm=new_area.hmNormalAvg(matrix, obj.mesh ? obj.mesh->ext : Extent(0.5)); // get new normal
