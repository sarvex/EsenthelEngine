...
class MergeSimilarMaterials : PropWin
{
   ...
   bool     name=false, color_name=false, color_is=false, color_tex=true, normal_tex=true, base2_tex=true, detail_tex=true, macro_tex=true, emissive_tex=true, tech=true, cull=true;
   flt      color_value=0.1, bump_value=0.1, smooth_value=0.1, reflect_value=0.1, glow_value=0.1, uv_scale=0.1;
   ...
   void create()
   {
      ...
      add("Smooth Value"     , MEMBER(MergeSimilarMaterials,  smooth_value)).desc("Tolerance").range(0, 1);
      add("Reflect Value"    , MEMBER(MergeSimilarMaterials, reflect_value)).desc("Tolerance").range(0, 1);
      ...
      FREPA(props)if(props[i].textline.is())
      {
         ...
         switch(c++)
         {
            ...
            case 2: check=& smooth_value_on; break;
            case 3: check=&reflect_value_on; break;
            case 4: check=&   glow_value_on; break;
