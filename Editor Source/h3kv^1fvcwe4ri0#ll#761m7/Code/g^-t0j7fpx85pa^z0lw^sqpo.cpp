...
class Project
{
   ...
   static int CompareID(C Elm &elm, C UID &id) {return Compare(elm.id, id);}
   ...
   Elm* findElm (C UID &id)  {return elms.addr(findElmI(id));}
 C Elm* findElm (C UID &id)C {return elms.addr(findElmI(id));}
   Elm&  getElm (C UID &id)  {int index; if(elms.binarySearch(id, index, CompareID))return elms[index]; Elm &elm=elms.NewAt(index); elm.id=id; return elm;} // get element of specified 'id', if it doesn't exist yet then create new one
   ...
   Elm& newElm(C Str &name, C UID &parent_id, ELM_TYPE type) {Elm &elm=newElm().setName(name, NewElmTime).setParent(parent_id, NewElmTime); elm.type=type; return elm;} // use custom times for 'setName' and 'setParent' so those times don't need to be sent to the server in the new element command, instead they will always be assumed 'NewElmTime' (check 'decompressNew')
   ...
   UID  findElmID(C Str &text) {if(Elm *elm=findElm(text))return elm.id; return UIDZero;}

   Elm* findElm  (C UID &id  , ELM_TYPE type) {if(  Elm *elm=findElm(id  ))if(elm.type==type)return elm   ; return null   ;}
 C Elm* findElm  (C UID &id  , ELM_TYPE type)C{if(C Elm *elm=findElm(id  ))if(elm.type==type)return elm   ; return null   ;}
   Elm* findElm  (C Str &text, ELM_TYPE type) {if(  Elm *elm=findElm(text))if(elm.type==type)return elm   ; return null   ;}
   UID  findElmID(C UID &id  , ELM_TYPE type) {if(  Elm *elm=findElm(id  , type            ))return elm.id; return UIDZero;}
   UID  findElmID(C Str &text, ELM_TYPE type) {if(  Elm *elm=findElm(text, type            ))return elm.id; return UIDZero;}

   Elm* findElmImage  (C Str &text) {if(Elm *elm=findElm     (text))if(ElmImageLike(elm.type))return elm   ; return null   ;}
   UID  findElmImageID(C Str &text) {if(Elm *elm=findElmImage(text                          ))return elm.id; return UIDZero;}
   ...
   Elm* findElmByTexture(C UID &tex_id)
   {
      ...
      return ret;
           ...
           Str elmFullName(C UID &elm_id, int max_elms=-1)C {if(C Elm *elm=findElm(elm_id))return elmFullName(elm, max_elms); return elm_id.valid() ? UnknownName : S;}
   ...
   Str basePath       (C Elm &elm   )C {return elm.id.valid() ? (ElmEdit(elm.type) ? edit_path : game_path)+EncodeFileName(elm.id)         : S;}
   Str codeBasePath   (C UID &elm_id)C {return elm_id.valid() ?                      code_base_path        +EncodeFileName(elm_id)+CodeExt : S;}
   Str codePath       (C UID &elm_id)C {return elm_id.valid() ?                      code_path             +EncodeFileName(elm_id)+CodeExt : S;}
   Str editPath       (C UID &elm_id)C {return elm_id.valid() ?                      edit_path             +EncodeFileName(elm_id)         : S;}
   Str gamePath       (C UID &elm_id)C {return elm_id.valid() ?                                  game_path +EncodeFileName(elm_id)         : S;}
   Str codeBasePath   (C Elm &elm   )C {return codeBasePath(elm.id);}
   Str codePath       (C Elm &elm   )C {return     codePath(elm.id);}
   Str editPath       (C Elm &elm   )C {return     editPath(elm.id);}
   Str gamePath       (C Elm &elm   )C {return     gamePath(elm.id);}
   Str  texPath       (C UID &tex_id)C {return tex_id.valid() ?                                    tex_path +EncodeFileName(tex_id) : S;}
   Str  texDynamicPath(C UID &tex_id)C {return tex_id.valid() ?                       temp_tex_dynamic_path +EncodeFileName(tex_id) : S;}
   ...
   Str gameWaypointPath(C UID &world_id, C UID &waypoint_id)C {return game_path+EncodeFileName(world_id)+"\\Waypoint\\"+EncodeFileName(waypoint_id);}
   Str editWaypointPath(C UID &world_id, C UID &waypoint_id)C {return edit_path+EncodeFileName(world_id)+"\\Waypoint\\"+EncodeFileName(waypoint_id);}

   bool waypointExists(C UID &world_id, C UID &waypoint_id)C {return FExist(gameWaypointPath(world_id, waypoint_id));} // waypoint game file can be deleted when it was removed so we need to check for file existence

   static Str EditLakePath (C Str &edit_path, C UID &world_id, C UID & lake_id)  {return edit_path+EncodeFileName(world_id)+"\\Lake\\" +EncodeFileName( lake_id);}
   static Str EditRiverPath(C Str &edit_path, C UID &world_id, C UID &river_id)  {return edit_path+EncodeFileName(world_id)+"\\River\\"+EncodeFileName(river_id);}
          Str editLakePath (                  C UID &world_id, C UID & lake_id)C {return EditLakePath (edit_path, world_id,  lake_id);}
          Str editRiverPath(                  C UID &world_id, C UID &river_id)C {return EditRiverPath(edit_path, world_id, river_id);}

   Str       worldVerPath   (C UID &world_id)C{return editPath(world_id)+WorldVerSuffix;}
   WorldVer* worldVerFind   (C UID &world_id) {return world_vers.find(worldVerPath(world_id));}
   WorldVer* worldVerGet    (C UID &world_id) {return world_vers.get (worldVerPath(world_id));}
   WorldVer* worldVerRequire(C UID &world_id) {return world_vers     (worldVerPath(world_id));}

   Str         miniMapVerPath   (C UID &mini_map_id)C{return editPath(mini_map_id);}
   MiniMapVer* miniMapVerFind   (C UID &mini_map_id) {return mini_map_vers.find(miniMapVerPath(mini_map_id));}
   MiniMapVer* miniMapVerGet    (C UID &mini_map_id) {return mini_map_vers.get (miniMapVerPath(mini_map_id));}
   MiniMapVer* miniMapVerRequire(C UID &mini_map_id) {return mini_map_vers     (miniMapVerPath(mini_map_id));}

   UID physToMesh(C Elm *phys) {if(phys)if(C ElmPhys *phys_data=phys.physData())return phys_data.mesh_id; return UIDZero;}
   ...
   UID animToObj (C Elm *anim) {return meshToObj (animToMesh(anim));}
   ...
   void savedGame(Elm &elm, C Str &name) {elm.file_size=FSize(name);}
   void savedGame(Elm &elm             ) {savedGame(elm, gamePath(elm));}
   ...
   void rebuildEmbedForObj (C    UID  &obj ) {Memt<UID> objs, exts; objs.add(obj); getExtendedObjs(objs, exts); rebuildEmbedForObjs(exts);}
   ...
   void rebuildPathsForObj (C    UID  &obj , bool only_not_ovr) {Memt<UID> objs, exts; objs.add(obj); getExtendedObjs(objs, exts); rebuildPathsForObjs(exts, only_not_ovr);}
   ...
   class AreaSyncObjData
   {
      ...
      AreaSyncObjData(Project &proj, C UID &world_id, WorldVer &world_ver) : proj(proj), world_id(world_id), world_ver(world_ver) {}
   }
   class AreaSyncObj
   {
      bool          changed=false;
      VecI2         xy=0;
      AreaVer      *area_ver=null;
   ...
   bool loadOldSettings (C Str &name)  {File f; return f.readTry(name) && loadOldSettings(f);}
   bool loadOldSettings2(C Str &name)  {return loadOldSettings(name) || loadOldSettings(name+".old");}
   ...
   LOAD_RESULT load(C Str &name, int &ver, SAVE_DATA mode=SAVE_ALL)  {File f; if(f.readTry(name))return load(f, ver, false, mode); ver=-1; del(); return LOAD_EMPTY;}
   bool        save(C Str &name                                   )C {File f; save(f.writeMem()); f.pos(0); return SafeOverwrite(f, name);}

   LOAD_RESULT load2(C Str &name, int &ver, SAVE_DATA mode=SAVE_ALL)  {LOAD_RESULT result=load(name, ver, mode); if(result==LOAD_EMPTY || result==LOAD_ERROR)result=load(name+".old", ver, mode); return result;}
   bool        save2(C Str &name                                   )C {if(FExistSystem(name))if(!FRename(name, name+".old"))return false; return save(name);}
...
class ElmNode
{
   bool      added=false; // if this element was added
   byte      flag=0; // ELM_FLAG
   int       parent=-1; // index of parent in hierarchy
   Memc<int> children; // valid indexes of elms

   void clear() {added=false; flag=0; parent=-1; children.clear();}
}
class ProjectHierarchy : Project
{
   ...
   virtual Elm* findElmByPath(C Str &path)override
   {
      ...
      return null;
           ...
           Str elmFullName(C UID &elm_id, int max_elms=-1)C {return super.elmFullName(elm_id, max_elms);}
