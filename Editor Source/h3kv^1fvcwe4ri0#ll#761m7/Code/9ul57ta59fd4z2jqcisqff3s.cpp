...
void SetExit(bool all_saved=true, ptr=null) {if(all_saved)StateExit.set();}
...
{
   Rect r=D.rect();
   if(modes && Mode.visibleTabs())MIN(r.max.y,          Mode.rect().min.y+D.pixelToScreenSize(Vec2(0, 0.5)).y);
   if(         Proj.visible    ())MAX(r.min.x,          Proj.rect().max.x-D.pixelToScreenSize(Vec2(0.5, 0)).x);
...
bool Init()
{
   if(InstallerMode)StateInstall.set();else
   {
      ...
      Gui+=Calculator.create(Rect_C(0, 0, 1.0, 0.235)).hide();
      AppStore.create();
      MSM.create();
      DST.create();
      CreateMtrls.create();
