...
bool Load (Mesh       &mesh, C Str &name, C Str &resource_path=S) {File f; if(f.read(name))return mesh.load(f, resource_path.is() ? resource_path : GetPath(name)); mesh.del  (); return false;}
bool Load (Material   &mtrl, C Str &name, C Str &resource_path=S) {File f; if(f.read(name))return mtrl.load(f, resource_path.is() ? resource_path : GetPath(name)); mtrl.reset(); return false;}
bool Load (WaterMtrl  &mtrl, C Str &name, C Str &resource_path=S) {File f; if(f.read(name))return mtrl.load(f, resource_path.is() ? resource_path : GetPath(name)); mtrl.reset(); return false;}
bool Load (EditObject &obj , C Str &name, C Str &resource_path=S) {File f; if(f.read(name))return obj .load(f, resource_path.is() ? resource_path : GetPath(name)); obj .del  (); return false;}
...
void SetTransform(Str &file, C Str &name, C Str &value=S) // this tries first to adjust existing transform
{
   Mems<FileParams> files=FileParams.Decode(file);
