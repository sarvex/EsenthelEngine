...
class ServerClass : Connection
{
   ...
   bool                           after_connect=false, version_sent=false, version_ok=false, logged_in=false, reconnect=false, allow_reconnect=false, proj_opened=false, send_proj_settings=false;
   USER_ACCESS                    access=UA_NO_ACCESS;
   ACTION                         action=NONE;
   SockAddr                       conn_addr;
   Str                            conn_email, conn_pass, conn_name, logged_email;
   uint                           conn_pass_key=0, sending=0, rcving=0;
   long                           sent=0, rcvd=0, stats_sent=0, stats_rcvd=0;
   flt                            reconnect_time=0, stats_time=0;
   Memx<Project>                  projects;
   Memc<UID>                      texs; // textures available on the server
   ThreadSafeMap<UID,   WorldVer>    world_vers(Compare); // element exists only if we have received data from server (even empty), make thread-safe just in case
   ThreadSafeMap<UID, MiniMapVer> mini_map_vers(Compare); // element exists only if we have received data from server (even empty), make thread-safe just in case

   // get
   bool loggedIn    ()C {return logged_in;}
   bool canRead     ()C {return loggedIn() && proj_opened;}
   bool canWrite    ()C {return loggedIn() && proj_opened && CanWrite    (access);}
   bool canWriteCode()C {return loggedIn() && proj_opened && CanWriteCode(access);}
   bool smallBuf    ()C {return queued()<=ServerSendBufSize;} // if buffer usage is small

   long sentTotal()C {return sent+super.sent    ();}
   long rcvdTotal()C {return rcvd+super.received();}
   ...
   void licenseKey        (C Str &license_key              ) {if(loggedIn()) ClientSendLicenseKey    (T, license_key);}
   void projectOpen       (C UID &proj_id, C Str &proj_name) {if(loggedIn()){ClientSendProjectOpen   (T, proj_id, proj_name); proj_opened=proj_id.valid();}}
   void projectDataRequest(                                ) {if(canRead ()) ClientSendGetProjectData(T);}
   void projectSetSettings(                                ) {if(canWrite())send_proj_settings=true;}
   void syncCodes         (  Memc<ElmTypeVer > &elms       ) {if(canRead ())ClientSendGetCodeVer (T, elms);}
   void syncCodes         (C Memc<ElmCodeData> &elms       ) {if(canWrite())ClientSendSetCodeData(T, elms);}

   void    newElm   (     Elm  &elm                                                         ) {if(canWrite())ClientSendNewElm      (T, elm );}
   void renameElm   (     Elm  &elm                                                         ) {if(canWrite())ClientSendRenameElm   (T, elm.id, elm.name     , elm.  name_time);}
   void setElmParent(     Elm  &elm                                                         ) {if(canWrite())ClientSendSetElmParent(T, elm.id, elm.parent_id, elm.parent_time);}
   void  removeElms (Memc<UID> &elms, bool  removed,                       C TimeStamp &time) {if(canWrite())ClientSendRemoveElms  (T, elms  , removed,                 time);}
   void publishElms (Memc<UID> &elms, sbyte publish, sbyte publish_mobile, C TimeStamp &time) {if(canWrite())ClientSendPublishElms (T, elms  , publish, publish_mobile, time);}
   void getElmNames (Memc<UID> &elms                                                        ) {if(canRead ())ClientSendGetElmNames (T, elms);}
   void getTextures (Memc<UID> &texs                                                        ) {if(canRead ())ClientSendGetTextures (T, texs);}
   void getElmShort (Memc<UID> &elms                                                        ) {if(canRead ())ClientSendGetElmShort (T, elms);}
   ...
   void setWaypoint(C UID &world_id, C UID &waypoint_id, C Version &waypoint_ver, EditWaypoint &waypoint) {if(canWrite())ClientSendSetWorldWaypoint(T, world_id, waypoint_id, waypoint_ver, waypoint);}
   void setLake    (C UID &world_id, C UID &    lake_id, C Version &    lake_ver, Lake         &lake    ) {if(canWrite())ClientSendSetWorldLake    (T, world_id,     lake_id,     lake_ver, lake    );}
   void setRiver   (C UID &world_id, C UID &   river_id, C Version &   river_ver, River        &river   ) {if(canWrite())ClientSendSetWorldRiver   (T, world_id,    river_id,    river_ver, river   );}

   void getWorldVer      (C UID &world_id                           ) {if(canRead() && world_id.valid() && !world_vers.find(world_id))ClientSendGetWorldVer      (T, world_id           );} // request if don't have yet
   void getWorldAreas    (C UID &world_id, Memc<AreaSync> &areas    ) {if(canRead() && world_id.valid()                              )ClientSendGetWorldAreas    (T, world_id, areas    );}
   void getWorldWaypoints(C UID &world_id, Memc<UID     > &waypoints) {if(canRead() && world_id.valid()                              )ClientSendGetWorldWaypoints(T, world_id, waypoints);}
   void getWorldLakes    (C UID &world_id, Memc<UID     > &lakes    ) {if(canRead() && world_id.valid()                              )ClientSendGetWorldLakes    (T, world_id, lakes    );}
   void getWorldRivers   (C UID &world_id, Memc<UID     > &rivers   ) {if(canRead() && world_id.valid()                              )ClientSendGetWorldRivers   (T, world_id, rivers   );}

   void getMiniMapVer     (C UID &mini_map_id                     ) {if(canRead() && mini_map_id.valid() && !mini_map_vers.find(mini_map_id))ClientSendGetMiniMapVer   (T, mini_map_id        );} // request if don't have yet
   void getMiniMapImages  (C UID &mini_map_id, Memc<VecI2> &images) {if(canRead() && mini_map_id.valid()                                    )ClientSendGetMiniMapImages(T, mini_map_id, images);}

   void setMiniMapSettings(C UID &mini_map_id, C Game.MiniMap.Settings &settings, C TimeStamp &settings_time) {if(canWrite() && mini_map_id.valid())ClientSendSetMiniMapSettings(T, mini_map_id, settings, settings_time);}
