...
class ElmName
{
   ...
   void set (Elm  &elm) {id=elm.id; time=elm.name_time; name=elm.name;}
   bool save(File &f  ) {f<<id<<time<<name; return f.ok();}
   bool load(File &f  ) {f>>id>>time>>name; return f.ok();}
}
...
class AreaSync
{
   ...
   void set(uint flag, C VecI2 &xy) {T.flag=flag; T.xy=xy;}
}
...
class ElmTypeVer
{
   ...
   void set(C UID &id, ELM_TYPE type, C Version &ver) {T.id=id; T.type=type; T.ver=ver;}
   void set(C Elm &elm                              ) {set(elm.id, elm.type, elm.data ? elm.data.ver : Version());}
...
class ElmCodeBase
{
   ...
   void set(C UID &id, C Str &data) {T.id=id; T.data=data;}

   bool save(File &f)C {f<<id<<data; return f.ok();}
   bool load(File &f)  {f>>id>>data; return f.ok();}
}
