...
class EditFont
{
   static cchar8 *Ascii="0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz!@#$%^&*()[]<>{}`~_-+=;:,.?/|\\'\" ";

   bool diagonal_shadow=true,
        clear_type=false, // works better when font is small
        software=false,
        ascii=true, german=false, french=false, polish=false, chinese=false, japanese=false, korean=false, russian=false;
  sbyte mip_maps=0;
   int  size=48;
   flt  scale=1, 
        weight=0,
