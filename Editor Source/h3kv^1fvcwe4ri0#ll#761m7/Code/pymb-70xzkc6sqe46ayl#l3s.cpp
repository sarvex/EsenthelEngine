...
class SynchronizerClass
{
   ...
   class ElmFile
   {
      bool compress=false, compress_fast=false;
      File elm, data;

      int size()C {return elm.size()+data.size();}
   ...
   {
      WorldVer   *server_ver=null, *local_ver=null;
      Memc<VecI2> set_area;
      Memc<UID  > set_obj;

      bool empty()C {return !set_area.elms() && !set_obj.elms();}
      int  elms ()C {return  set_area.elms() +   set_obj.elms();}

      void getServerVer(C UID &world_id) {if(!server_ver)server_ver=Server.world_vers.find(world_id);}
   }
   ...
   {
      MiniMapVer *server_ver=null, *local_ver=null;
      Memc<VecI2> set_image;

      bool empty()C {return !set_image.elms();}
      int  elms ()C {return  set_image.elms();}

      void getServerVer(C UID &mini_map_id) {if(!server_ver)server_ver=Server.mini_map_vers.find(mini_map_id);}
   }
   ...
   Memc<File                     > cmds;
   ThreadSafeMap<UID,   WorldSync>    world_sync(Compare, Create), delayed_world_sync(Compare, Create); // make thread-safe just in case
   ThreadSafeMap<UID, MiniMapSync> mini_map_sync(Compare, Create);                                      // make thread-safe just in case
   Str                             game_path, edit_path, tex_path;
   Thread                          thread;
   SyncLock                        lock;
   flt                             last_delayed_time=0;

  ~SynchronizerClass() {clearSync(); thread.del();} // delete thread before other members

   static bool CompressFunc(Thread &thread) {return Synchronizer.compressFunc();}
          bool compressFunc()
   {
   ...
   int elmFileSize() {int size=0; SyncLocker locker(lock); REPA(set_elm_full_file)size+=set_elm_full_file[i].size(); REPA(set_elm_long_file)size+=set_elm_long_file[i].size(); return size;}
   int queuedElms ()
   {
   ...
   void setElmFull (C UID &elm_id) {if(elm_id.valid()){/*SyncLocker locker(lock);*/                              set_elm_full .include(elm_id);}}
   void setElmLong (C UID &elm_id) {if(elm_id.valid()){/*SyncLocker locker(lock);*/                              set_elm_long .include(elm_id);}}
   void setElmShort(C UID &elm_id) {if(elm_id.valid()){/*SyncLocker locker(lock);*/ if(!set_elm_long.has(elm_id))set_elm_short.include(elm_id);}} // if we're already sending long version then skip short
   void setTex     (C UID &tex_id) {if(tex_id.valid()){  SyncLocker locker(lock);                                set_tex      .include(tex_id);}}
