...
class EditPanelImage : PanelImageParams
{
   ...
   class Section
   {
      ...
      UID         depth_overlay_id=UIDZero, color_overlay_id=UIDZero, reflection_id=UIDZero;
      ImageParams depth_overlay_params, color_overlay_params, depth_noise, color_noise;

      void reset() {T=Section();}
   ...
   class Base
   {
      ...
      bool      compressed=false, mip_maps=false; // mip-maps disabled by default, because when texture is stretched lower mip maps are used which can look as if outside of the rectangle
      UID       top_id=UIDZero, bottom_id=UIDZero, center_id=UIDZero, left_id=UIDZero, right_id=UIDZero, top_left_id=UIDZero, top_right_id=UIDZero, bottom_left_id=UIDZero, bottom_right_id=UIDZero;
      Light     lights  [2];
      Section   sections[2];
   ...
   void reset  (                     )  {T=EditPanelImage();}
   void newData(                     )  {       base.newData();}
   bool newer  (C EditPanelImage &src)C {return base.newer(src.base);}
   bool equal  (C EditPanelImage &src)C {return base.equal(src.base);}
   bool sync   (C EditPanelImage &src)  {return base.sync(T, src.base, src);}
   bool undo   (C EditPanelImage &src)  {return base.undo(T, src.base, src);}
   ...
   class Images
   {
      ...
      Images(Project &proj, bool fast) : proj(proj) {T.fast=fast;}
