...
class AreaBuild
{
   ...
   void process(BuilderClass &builder)
   {
      ...
      {
         if(builder.io_thread.wantStop())return;
         MeshBase temp; temp.createPhys   (hm_mesh).weldVtx(MESH_NONE, 0.01, EPS_COL_COS, -1).simplify(1, 0.10, 1, 1, 1, 1, PI, true, SIMPLIFY_PLANES);
      ...
      {
         if(builder.io_thread.wantStop())return;
         MeshBase temp; temp.createPhys   (hm_mesh).weldVtx(MESH_NONE, 0.01, EPS_COL_COS, -1).simplify(1, 0.05, 1, 1, 1, 1, PI, true, SIMPLIFY_PLANES);
                     hm_phys.createMeshTry(   temp);
      }
