...
class ProjectSettings : ClosableWindow
{
   ...
   static void CompressTypeChanged    (ProjectSettings &ps) {Proj.compress_type    =ps.compressType (); Proj.compress_type_time    .getUTC(); Server.projectSetSettings(); ps.toGui();}
   ...
   void create()
   {
      ...
      Gui+=super.create(Rect_C(0, 0, 1.28, 0.61), "Project Settings").hide(); button[2].func(HideProjAct, SCAST(GuiObj, T)).show();
      ...
      T+=tex_size_mobile.create(Rect_L(0.32, y, 0.3, h), MaterialRegion.TexDownsizeText, MaxMaterialDownsize).func(TexSizeMobileChanged, T).desc("This option allows to specify default value of \"Tex Size Mobile\" for newly created Materials."); y-=p;

      T+=tex_size_switch_t.create(Vec2(0.015, y), "Tex Size Switch", &ts);
      T+=tex_size_switch.create(Rect_L(0.32, y, 0.3, h), MaterialRegion.TexDownsizeText, MaxMaterialDownsize).func(TexSizeSwitchChanged, T).desc("This option allows to specify default value of \"Tex Size Switch\" for newly created Materials."); y-=p;
