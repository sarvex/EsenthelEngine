...
class CopyElements : ClosableWindow
{
   class ProjListElm // project as list element
   {
      ...
      ProjListElm(C Projects.Elm &src) {name=src.local_name; path=src.path; id=src.id;}
      ProjListElm(C Project      &src) {name=src.      name; path=src.path; id=src.id;}
   }
   ...
   class ReplaceElms : ClosableWindow
   {
      class ElmListElm // project element as list element
      {
         ...
         ElmListElm(C UID &elm_id) {T.id=elm_id; T.name=CopyElms.src.elmFullName(elm_id);}

         static int Compare(C ElmListElm &a, C ElmListElm &b) {return ComparePathNumber(a.name, b.name);}
      }
      ...
      static void Copy(ReplaceElms &re) {re.copyDo();}
             void copyDo()
      {
         hide();
         if(CopyElms.canCopy())
   ...
   int               src_ver=-1;
   ProjectHierarchy  temp_src , *src =null;
   Project           temp_dest, *dest=null;
   ProjListElm       target;
   UID               root=UIDZero;
   ...
   void create()
   {
      Gui+=super.create(Rect_C(0, 0, 1.8, 1.1), "Copy elements").hide(); button[2].func(HideProjAct, SCAST(GuiObj, T)).show();
      T+=proj_region.create(Rect_RU(clientWidth()-0.04, -0.10, 0.5, 0.82));
      T+=elms_region.create(Rect   (0.04, proj_region.rect().min.y, proj_region.rect().min.x-0.04, proj_region.rect().max.y));
      T+=elms_text  .create(elms_region.rect().up()+Vec2(0, 0.04), "Following elements will be copied");
      T+=proj_text  .create(proj_region.rect().up()+Vec2(0, 0.04), "Select target project");
      ListColumn elms_lc[]=
      {
      ...
      T+=include_children    .create(Rect_LD(                  0.04, -clientHeight()+0.04  , 0.38, 0.06), "Include children"    ).func(Refresh, T).focusable(false).desc("If include elements which are assigned as children to selected elements."                                                                                             ); include_children    .mode=BUTTON_TOGGLE; include_children    .set(true, QUIET);
      T+=include_dependencies.create(Rect_LU(include_children    .rect().ru()+Vec2(0.03, 0), 0.48, 0.06), "Include dependencies").func(Refresh, T).focusable(false).desc("If include elements on which selected elements depend on.\nFor example if a mesh is selected, then enabling this option will select all materials that the mesh uses."); include_dependencies.mode=BUTTON_TOGGLE; include_dependencies.set(true, QUIET);
      T+=include_parents     .create(Rect_LU(include_dependencies.rect().ru()+Vec2(0.03, 0), 0.48, 0.06), "Include parents"     ).func(Refresh, T).focusable(false).desc("If include elements which are assigned as parents of selected elements."                                                                                              ); include_parents     .mode=BUTTON_TOGGLE;
      T+=copy                .create(Rect_RD(clientWidth()-0.04, -clientHeight()+0.04      , 0.25, 0.06), "Copy"                ).func(Copy   , T);
      replace_elms.create();
   }
