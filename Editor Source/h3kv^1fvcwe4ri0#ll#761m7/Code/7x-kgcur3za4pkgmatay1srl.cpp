...
class EditMaterial
{
   ...
   bool                      flip_normal_y=false, smooth_is_rough=false, cull=true, detail_all_lod=false;
   ...
   EditMaterial() {REPAO(tex_downsize)=0;}
   ...
   void resetAlpha() {color_s.w=((HasAlphaTest(tech) && !HasAlphaBlend(tech) && !HasAlphaTestDither(tech)) ? 0.5 : 1); color_time.getUTC();}
   ...
   uint sync(C Edit.Material &src)
   {
      ...
      changed|=SyncByValue(     detail_all_lod_time, time, detail_all_lod     , src.detail_all_lod     )* CHANGED_PARAM;
   ...
   uint sync(C EditMaterial &src)
   {
      ...
      changed|=Sync(     detail_all_lod_time, src.     detail_all_lod_time, detail_all_lod     , src.detail_all_lod     )* CHANGED_PARAM;
   ...
   uint undo(C EditMaterial &src)
   {
      ...
      changed|=Undo(     detail_all_lod_time, src.     detail_all_lod_time, detail_all_lod     , src.detail_all_lod     )* CHANGED_PARAM;
   ...
   void adjustParams(uint changed, TEX_FLAG old_textures, TEX_FLAG has_textures, TEX_FLAG known_textures) // 'old_textures'=textures() before making any change, 'has_textures'=used textures based on per-pixel data (if known), 'known_textures'=what textures in 'has_textures' are known
   {
      ...
      if(known_textures&TEXF_ALPHA)
      {
         if(has_textures&TEXF_ALPHA)
         {
            ...
            {
            enable_alpha:
               if(!HasAlphaBlend(tech) && !HasAlphaTestDither(tech) && color_s.w>=1-EPS_COL8){color_s.w=0.5; color_time=time;}
