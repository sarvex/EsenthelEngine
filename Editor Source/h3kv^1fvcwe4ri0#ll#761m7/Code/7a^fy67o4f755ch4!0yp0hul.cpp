...
void SetCam(Camera &cam, C Box &box, flt yaw, flt pitch=0, flt zoom=1) // require custom 'cam' to calculate correct velocities
{
   cam.updateBegin();
   cam.yaw=yaw;
   cam.pitch=pitch;
...
class PreviewClass : Window // create as window so it will be above other windows in the editor
{
   ...
   static void Render() {Preview.render();}
          void render()
   {
      switch(Renderer())
      {
   ...
   void draw(Elm &elm, C Rect &rect)
   {
      ...
      switch(elm_type=elm.type)
      {
         case ELM_OBJ:
         {
            ...
            if(obj=Proj.gamePath(elm.id))
            {
               ...
               flt dist=Max(0.1, mesh ? GetDist(mesh->ext) : 0);
               D.viewFrom(dist*0.01).viewRange(dist*5); if(mesh)SetCam(cam, mesh->ext, Time.appTime(), (mesh->ext.size().y<mesh->ext.size().max()*0.3) ? -PI_4 : 0); // set
         ...
         case ELM_MTRL:
         {
            ...
            D.viewFrom(dist*0.01).viewRange(dist*3); SetCam(cam, ball.ext, Time.appTime()/2); // set
            Renderer(PreviewClass.Render);
            D.viewRect(r).viewFov(fov).viewFrom(from).viewRange(range); c.set(); // restore
            rect.draw(rect_color, false);
         }break;
         ...
         case ELM_WATER_MTRL:
         {
            ...
            D.viewRect(rect).viewFov(PreviewFOV).viewFrom(0.1).viewRange(100); SetCam(cam, Box(5), 0, -PI_6); // set
            Renderer(PreviewClass.Render);
            D.viewRect(r).viewFov(fov).viewFrom(from).viewRange(range); c.set(); // restore
         ...
         case ELM_ANIM:
         {
            ...
            if( mesh_id.valid())
            {
               ...
               D.viewRect(rect).viewFov(PreviewFOV);
               flt dist=Max(0.1, mesh ? GetDist(mesh->ext) : 0);
               D.viewFrom(dist*0.01).viewRange(dist*5); if(mesh)SetCam(cam, mesh->ext, Time.appTime()); // set
               Renderer(PreviewClass.Render);
               D.viewRect(r).viewFov(fov).viewFrom(from).viewRange(range); c.set(); // restore
               rect.draw(rect_color, false);
            }
         ...
         case ELM_PANEL:
         {
            drawBack(rect); if(PanelPtr panel=Proj.gamePath(elm.id))panel->draw(Rect_C(rect.center(), rect.size()*0.5));
         }break;
         ...
         case ELM_GUI_SKIN:
         {
            if(GuiSkinPtr skin=Proj.gamePath(elm.id))
            {
               ...
               Window window; window.create(Rect_C(0, 0, rect.w()*0.75, rect.h()*0.5), "Window");
               Text   text  ; text  .create(window.clientRect().lerp(0.5, 0.65), "Hello!");
               Button button; button.create(Rect_C(window.clientRect().lerp(0.5, 0.35), 0.3, 0.06), "OK");
               GuiPC gpc;
         ...
         case ELM_IMAGE_ATLAS:
         {
            ...
            if(atlas=Proj.gamePath(elm.id))if(int parts=atlas->parts.elms())
            {
               ...
               if(InRange(part.image_index, atlas->images))
               {
                  ...
                  Vec2 scale=tex_size/part.original_size*0.9;
         ...
         case ELM_ENV:
         {
            ...
            D.viewRect(rect).viewFov(PreviewFOV).viewFrom(0.01).viewRange(300); SetCam(cam, Box(0), Time.appTime()/2); // set
            Renderer(PreviewClass.Render);
            D.viewRect(r).viewFov(fov).viewFrom(from).viewRange(range); c.set(); // restore
            rect.draw(rect_color, false);
         }break;
         ...
         case ELM_MINI_MAP:
         {
            ...
            for(int x=-4; x<4; x++)
            {
               ...
               if(img.is())img.draw(Rect_LD(x, y, 1, 1)*0.19+rect.center());
            }
            D.clip();
         }break;
      ...
      if(elm.importing()){TextStyleParams ts; ts.size*=0.6; ts.align.set(0, 1); D.text(ts, rect.down(), (ImportRemovedElms || elm.finalExists()) ? "Processing" : "Not Processed");}
   }
   ...
   {
      D.clip();
      Vec2 size(D.w(), D.h()); size*=0.5; size=size.avg();
      Rect r;
      if(rect.min.x-(-D.w())>D.w()-rect.max.x)r=Rect_R(rect.min.x-0.02, y, size.x, size.y);
      else                                    r=Rect_L(rect.max.x+0.02, y, size.x, size.y);
      clipRect(r);
      draw(elm, r);
   }
   ...
   virtual void    draw  (C GuiPC &gpc)override
   {
      if(ListElm *list_elm=Proj.list.visToData(Proj.list.lit))
      {
         ...
         {
            D.clip();
            Vec2   size(D.w(), D.h()); size*=0.75; size=size.avg(); if(elm.type==ELM_SOUND)size.set(0.38, 0.31);
      ...
      {
         D.clip();
         Vec2   size(D.w(), D.h()); size*=0.75; size=size.avg();
         Rect   rect=MSM.region.screenRect();
         if(rect.min.x-(-D.w())>D.w()-rect.max.x)rect=Rect_R(rect.min.x-0.02, Ms.pos().y, size.x, size.y);
         else                                    rect=Rect_L(rect.max.x+0.02, Ms.pos().y, size.x, size.y);
         clipRect(rect);
         draw(*elm, rect);
      }
