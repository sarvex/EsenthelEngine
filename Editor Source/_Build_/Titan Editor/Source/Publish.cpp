...
/******************************************************************************/
   ...
   void PublishResult::display(C Str &text)
   {
      ...
      T+=ok        .create(Rect_D(clientWidth()*(PublishProjectPackage ? 2 : 1)/4, -clientHeight()+0.04f, 0.3f, 0.06f), "OK"        ).func(OK       , T);
      T+=size_stats.create(Rect_D(clientWidth()*                             3 /4, -clientHeight()+0.04f, 0.3f, 0.06f), "Size Stats").func(SizeStats, T).hidden(PublishProjectPackage);
