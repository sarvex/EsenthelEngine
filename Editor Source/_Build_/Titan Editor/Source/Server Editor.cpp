...
/******************************************************************************/
   void EditorServer::ConvertHeight(C Heightmap &src, Image &dest, flt area_size)
   {
   ...
   void EditorServer::ConvertMaterial(C Heightmap &src, Edit::MaterialMap &dest)
   {
   ...
   void EditorServer::ConvertMaterial(Edit::MaterialMap &src, Heightmap &dest) // 'src' will get modified !!
   {
      ...
      bool EditorServer::Client::update()
{
                                    ...
                                    if(src.align_to_terrain || src.align_to_terrain_normal>EPS) // want to align
                                    {
                                       ...
                                       if(hm)
                                       {
                                          ...
                                          if(src.align_to_terrain_normal>EPS) // align to terrain normal
                                          {
                                             ...
                                             Vec normal=HmNormalAvg(*hm, area_size, area_xy, dest.matrix, mesh ? mesh->ext : Extent(0.5f));
   ...
   void EditorServer::update(bool busy)
   {
      T.busy    =busy;
      T.received=false;
      super::update();
...
EditorServer::Client::Client() : queued(-1) {}
