...
/******************************************************************************/
   ...
   void ProjectSettings::CompressTypeChanged(ProjectSettings &ps) {Proj.compress_type [PCP_DEFAULT]=ps.compressType         (); Proj.compress_time[PCP_DEFAULT].getUTC(); Server.projectSetSettings(); ps.toGui();}
   void ProjectSettings::CompressLevelChanged(ProjectSettings &ps) {Proj.compress_level[PCP_DEFAULT]=ps.compress_level       (); Proj.compress_time[PCP_DEFAULT].getUTC(); Server.projectSetSettings();}
   void ProjectSettings::CompressTypeMobileChanged(ProjectSettings &ps) {Proj.compress_type [PCP_MOBILE ]=ps.compressTypeMobile   (); Proj.compress_time[PCP_MOBILE ].getUTC(); Server.projectSetSettings(); ps.toGui();}
   void ProjectSettings::CompressLevelMobileChanged(ProjectSettings &ps) {Proj.compress_level[PCP_MOBILE ]=ps.compress_level_mobile(); Proj.compress_time[PCP_MOBILE ].getUTC(); Server.projectSetSettings();}
   ...
   void ProjectSettings::TexSizeMobileChanged(ProjectSettings &ps) {Proj.tex_downsize[TSP_MOBILE]=ps.tex_size_mobile(); Proj.tex_downsize_time.getUTC(); Server.projectSetSettings();}
   void ProjectSettings::TexSizeSwitchChanged(ProjectSettings &ps) {Proj.tex_downsize[TSP_SWITCH]=ps.tex_size_switch(); Proj.tex_downsize_time.getUTC(); Server.projectSetSettings();}
   ...
   void ProjectSettings::create()
   {
      ...
      Gui+=super::create(Rect_C(0, 0, 1.31f, 0.68f), "Project Settings").hide(); button[2].func(HideProjAct, SCAST(GuiObj, T)).show();
      ...
      T+=cipher_type.create(Rect_L(0.01f, y, 0.29f, h)).func(CipherChanged, T).desc(cipher_desc); cipher_type.setColumns(class_name_desc_column, Elms(class_name_desc_column), true).setData(CipherText, CIPHER_NUM).set(0, QUIET).menu.list.setElmDesc(MEMBER(ClassNameDesc, desc));
      T+=cipher_key.create(Rect_L(x1, y, 0.73f, h)).func(EncryptionKeyChanged, T).desc("256 Encryption key numbers in \"X, X, X, ..\" format");
      T+=randomize_key.create(Rect_LU(cipher_key.rect().ru(), 0.21f, cipher_key.rect().h()), "Randomize").func(RandomizeEncryptionKey, T); y-=p;
      ...
      T+=tex_size_mobile.create(Rect_L(x1, y, 0.3f, h), MaterialRegion::TexDownsizeText, MaxMaterialDownsize).func(TexSizeMobileChanged, T).desc("This option allows to specify default value of \"Tex Size Mobile\" for newly created Materials."); y-=p;

      T+=tex_size_switch_t.create(Vec2(0.015f, y), "Tex Size Switch", &ts);
      T+=tex_size_switch.create(Rect_L(x1, y, 0.3f, h), MaterialRegion::TexDownsizeText, MaxMaterialDownsize).func(TexSizeSwitchChanged, T).desc("This option allows to specify default value of \"Tex Size Switch\" for newly created Materials."); y-=p;
