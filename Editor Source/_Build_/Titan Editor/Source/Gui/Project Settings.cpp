...
/******************************************************************************/
   ...
   void ProjectSettings::CompressTypeChanged(ProjectSettings &ps) {Proj.compress_type    =ps.compressType (); Proj.compress_type_time    .getUTC(); Server.projectSetSettings(); ps.toGui();}
   ...
   COMPRESS_TYPE ProjectSettings::compressType() {return InRange(compress_type(), cmpr_type) ? cmpr_type[compress_type()].type : COMPRESS_NONE;}
   int           ProjectSettings::compressLevel() {return compress_level();}
   ...
   void ProjectSettings::create()
   {
      ...
      Gui+=super::create(Rect_C(0, 0, 1.28f, 0.61f), "Project Settings").hide(); button[2].func(HideProjAct, SCAST(GuiObj, T)).show();
      ...
      T+=cipher_key.create(Rect_L(0.32f, y, 0.73f, h)).func(EncryptionKeyChanged, T).desc("256 Encryption key numbers in \"X, X, X, ..\" format");
