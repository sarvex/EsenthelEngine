      ...
      void ImageEditor::Change::apply(ptr user)
{
   ...
   void ImageEditor::Mode(  ImageEditor &ie, C Str &t) {if(ElmImage *d=ie.data()){int i=TextInt(t); if(InRange(i, ImageModes)){d->mode=ImageModes[i].mode; d->mode_time.getUTC(); d->env(ImageModes[i].env); d->env_time.getUTC();}}}
   Str  ImageEditor::Mode(C ImageEditor &ie          ) {if(ElmImage *d=ie.data())REPA(ImageModes)if(d->mode==ImageModes[i].mode && d->envActual()==ImageModes[i].env)return i; return -1;}
   ...
   void ImageEditor::setInfo()
   {
      ...
      if(  width   )  width   ->display(S+"Width: "   +         (game_image ? game_image->w           ()      : 0));
      if(  height  )  height  ->display(S+"Height: "  +         (game_image ? game_image->h           ()      : 0));
      if(  type    )  type    ->display(S+"Type: "    +         (game_image ? game_image->typeInfo    ().name : null));
      if(i_mip_maps)i_mip_maps->display(S+"Mip Maps: "+         (game_image ? game_image->mipMaps     ()      : 0));
      if(  mem_size)  mem_size->display(S+"Size: "    +SizeBytes(game_image ? game_image->typeMemUsage()      : 0)); // use 'typeMemUsage' because we need this only for stats
...
ImageEditor::ImageEditor() : elm_id(UIDZero), elm(null), changed(false), no_scale(false), source(true), channels(0), mip_maps(null), width(null), height(null), type(null), i_mip_maps(null), mem_size(null), chn(null), nos(null), src(null), undos(true) {}
