...
{
   mesh.setBase().simplify(1, 0.05f, 1, 0.02f, 0.02f, 1, PI, true, SIMPLIFY_PLANES);
...
/******************************************************************************/
   void EmbedObjBase::operator=(ObjVer &obj)
   {
   ...
   void EmbedObj::set(C Matrix &matrix, C Object &src, bool set_mesh, bool set_phys, OBJ_PATH *ovr_path, uint *ovr_mesh_variation_id) // this is called on secondary thread
   {
      MeshPtr     mesh_ptr;
      ...
      void AreaBuild::PhysObj::set(C Obj &src) // this is called on main thread
      {
      ...
      void AreaBuild::PhysObj::set(C Object &src) // this is called on secondary thread
      {
         CacheLock cl(Objects); // use lock so 'Object' members will not be changed on secondary thread
            path =src.path();
         if(path!=OBJ_PATH_IGNORE)
      ...
      void AreaBuild::PhysObj::set(C ObjData &src, Memt<ObjectPtr> &obj_cache) // this is called on secondary thread
      {
            path =(src.removed ? OBJ_PATH_IGNORE : src.params.path);
         if(path!=OBJ_PATH_IGNORE)
         {
   ...
   bool AreaBuild::Create(AreaBuild &area, C VecI2 &xy, ptr world_ver_ptr)
   {
   ...
   void AreaBuild::SetPathType(MeshBase &mesh, PATH_MESH_TYPE path_mode)
   {
   ...
   void AreaBuild::SetDefaultPathType(Memt<MeshBase> &meshes)
   {
   ...
   void AreaBuild::process(BuilderClass &builder)
   {
   ...
   void AreaBuild::processPath(BuilderClass &builder)
   {
   ...
   void BuilderClass::Process(AreaBuild &area, BuilderClass &builder, int thread_index) {area.process    (builder);}
   void BuilderClass::ProcessPath(AreaBuild &area, BuilderClass &builder, int thread_index) {area.processPath(builder);}
   bool BuilderClass::IOFunc(Thread &thread) {return ((BuilderClass*)thread.user)->ioFunc();}
          bool BuilderClass::ioFunc(              )
   {
  ...
  BuilderClass::~BuilderClass() {io_thread.stop(); io_wait.on(); io_thread.del();}
   bool BuilderClass::processing(                    ) {return areas.elms()>0;}
   bool BuilderClass::finished(C UID *world_id) {return !processing() && !Proj.worldAreasToRebuild(world_id);}
   void BuilderClass::stop()
   {
   ...
   bool BuilderClass::process(WorldVer &world_ver, C ElmWorld &world_data, VecI2 area_xy)
   {
   ...
   void BuilderClass::update(bool background, C UID *priority_world_id)
   {
...
EmbedObjBase::EmbedObjBase() : ovr_path(false), ovr_mesh_variation_id(false), mesh_variation_id(0), path(OBJ_PATH_CREATE) {}

EmbedObj::EmbedObj() : path(OBJ_PATH_CREATE) {}

AreaBuild::AreaBuild() : xy(0), build(0), load(0), l(null), r(null), b(null), f(null), lb(null), lf(null), rb(null), rf(null), world_ver(null) {}

AreaBuild::PhysObj::PhysObj() : path(OBJ_PATH_IGNORE), matrix(1) {}

BuilderClass::BuilderClass() : finished_processing(false), background(false), areas(Compare, AreaBuild::Create), embed_objs_rect(0, -1), water_rect(0, -1), world_ver(null) {}
