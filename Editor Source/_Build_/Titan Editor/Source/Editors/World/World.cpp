...
/******************************************************************************/
   ...
   cchar8 *WorldView::waypoint_loop_t[]=
   {
      "Single"   , // 0
      "Loop"     , // 1
      "Ping-Pong", // 2
...
/******************************************************************************/
   WorldData::WorldData() : elm_id(UIDZero), elm(null), area_size(64), hm_res(64), ver(null) {grid.user(this);}
   flt WorldData::areaSize()C {return area_size;}
   int   WorldData::hmRes()C {return hm_res;}
   Vec2   WorldData::areaToWorld(C VecI2 &xz  )C {return                                      xz*areaSize() ;}
   VecI2 WorldData::worldToArea(C Vec2  &xz  )C {return                                Floor(xz/areaSize());}
   VecI2 WorldData::worldToArea(C Vec   &pos )C {return                              worldToArea(pos .xz());}
   RectI WorldData::worldToArea(C Rect  &rect)C {return RectI(worldToArea(rect.min), worldToArea(rect.max));}
   RectI WorldData::worldToArea(C Box   &box )C {return RectI(worldToArea(box .min), worldToArea(box .max));}
   Area*  WorldData::getArea(C VecI2 &area_xy) {return              grid.get (area_xy).data();}
   Area* WorldData::findArea(C VecI2 &area_xy) {if(Cell<Area> *area=grid.find(area_xy))return area->data(); return null;}
   Area* WorldData::findAreaLoaded(C VecI2 &area_xy) {if(     Area  *area=findArea (area_xy))if(area->loaded)return area; return null;}
   void WorldData::load()
   {
      ver=null; 
      if(elm)ver=Proj.worldVerRequire(elm_id);
   }
   void WorldData::flush()
   {
      if(elm)grid.func(Area::Flush);
      if(ver)ver->flush();
   }
   void WorldData::unload()
   {
      flush();
      grid.del();
      path_world.del();
   ...
   void WorldView::Grid(WorldView &world) {world.show_grid.push();}
   void WorldView::GridLevel(WorldView &world) {world.grid_plane_level_win.visibleToggleActivate();}
   void WorldView::GotoArea(WorldView &world) {world.goto_area.visibleToggleActivate();}
   void WorldView::HmAddRem(WorldView &world) {SetHmOp(world.hm_add_rem() ? OP_HM_ADD_REM : OP_HM_NONE);}
   void WorldView::HmSelMtrl(WorldView &world) {}
   void WorldView::HmImport(WorldView &world) {ImportTerrain.visibleToggleActivate();}
   void WorldView::DecRadius(WorldView &world) {world.visibleRadius(world.visibleRadius()-1);}
   void WorldView::IncRadius(WorldView &world) {world.visibleRadius(world.visibleRadius()+1);}
   void WorldView::DecSel(WorldView &world) {world.hmSelSize    (world.hmSelSize    ()-1);}
   void WorldView::IncSel(WorldView &world) {world.hmSelSize    (world.hmSelSize    ()+1);}
   void WorldView::Mode1(WorldView &world) {world.mode.toggle(0);}
   void WorldView::Mode2(WorldView &world) {world.mode.toggle(1);}
   void WorldView::Mode3(WorldView &world) {world.mode.toggle(2);}
   void WorldView::Mode4(WorldView &world) {world.mode.toggle(3);}
   void WorldView::Mode5(WorldView &world) {world.mode.toggle(4);}
   void WorldView::ModeS0(WorldView &world) {world.modeS(0);}
   void WorldView::ModeS1(WorldView &world) {world.modeS(1);}
   void WorldView::ModeS2(WorldView &world) {world.modeS(2);}
   void WorldView::ModeS3(WorldView &world) {world.modeS(3);}
   void WorldView::ModeS4(WorldView &world) {world.modeS(4);}
   void WorldView::ModeS5(WorldView &world) {world.modeS(5);}
   void WorldView::ModeS6(WorldView &world) {world.modeS(6);}
   void WorldView::ModeS7(WorldView &world) {world.modeS(7);}
   void WorldView::ModeS8(WorldView &world) {world.modeS(8);}
   void WorldView::ModeS9(WorldView &world) {world.modeS(9);}
   void WorldView::ModeS10(WorldView &world) {world.modeS(10);}
   void WorldView::ModeS11(WorldView &world) {world.modeS(11);}
   ...
   void WorldView::ShowObjList(WorldView &world) {world.obj_list .push();}
   void WorldView::ShowObjPaint(WorldView &world) {world.obj_paint.push();}
   void WorldView::ShowObjPoint(WorldView &world) {world.obj_matrix_points=world.show_menu.menu("Object Points");}
   void WorldView::ShowObjBox(WorldView &world) {world.show_obj_box     =world.show_menu.menu("Object Boxes");}
   void WorldView::ShowObjPhys(WorldView &world) {world.show_obj_phys    =world.show_menu.menu("Physical Bodies");}
   void WorldView::ShowWorldPath(WorldView &world) {world.show_world_path  =world.show_menu.menu("World Path Mesh");}
   void WorldView::ShowWorldCenter(WorldView &world) {world.show_world_center=world.show_menu.menu("World Center");}
   void WorldView::ShowObjs(WorldView &world) {world.show_objs                          =world.show_menu.menu("Objects/All"    ); world.setObjVisibility();}
   void WorldView::ShowObjTerrain(WorldView &world) {world.show_obj_access[OBJ_ACCESS_TERRAIN]=world.show_menu.menu("Objects/Terrain"); world.setObjVisibility();}
   void WorldView::ShowObjGrass(WorldView &world) {world.show_obj_access[OBJ_ACCESS_GRASS  ]=world.show_menu.menu("Objects/Grass"  ); world.setObjVisibility();}
   void WorldView::ShowObjCustom(WorldView &world) {world.show_obj_access[OBJ_ACCESS_CUSTOM ]=world.show_menu.menu("Objects/Custom" ); world.setObjVisibility();}
   void WorldView::ShowObjClass(UID       &id   ) {WorldEdit.hide_obj_classes.binaryToggle(id);                                   WorldEdit.setObjVisibility();}
   void WorldView::ShowWaypointList(WorldView &world) {world.waypoint_list.push();}
   void WorldView::ModeChanged(WorldView &world) {world.modeChanged();}
   void WorldView::ObjOpChanged(WorldView &world) {world.obj_pos.apply();}
   void WorldView::WaterOpChanged(WorldView &world) {world.setWaterVis();}
   void WorldView::Play(WorldView &world) {if(world.elm)StartGame(*world.elm);}
   void WorldView::Undo(WorldView &world) {world.undos.undo();}
   void WorldView::Redo(WorldView &world) {world.undos.redo();}
   void WorldView::Locate(WorldView &world) {Proj.elmLocate(world.elm_id);}
   void WorldView::modeS(int i)
   {
      switch(mode())
      {
   ...
   void WorldView::AreaDrawPrepare(Cell<Area> &cell, ptr) {cell().draw      ();}
   void WorldView::AreaDrawShadow(Cell<Area> &cell, ptr) {cell().drawShadow();}
   void WorldView::AreaDraw2D(Cell<Area> &cell, ptr) {cell().draw2D    ();}
   void WorldView::Render() {WorldEdit.render();}
   ...
   void WorldView::Draw(Viewport &viewport) {if(Edit::Viewport4::View *view=WorldEdit.v4.getView(&viewport))WorldEdit.draw(*view);}
          void WorldView::draw(Edit::Viewport4::View &view)
   {
      ...
      flt frac=D.shadowFrac(); D.shadowFrac(frac*1.05f*dist/D.viewRange());
      ...
      if(show_world_center){SetMatrix(); Matrix(ActiveCam.dist*0.07f*2*Tan(D.viewFovY()/2)).draw();}
      ...
      if(mode()==WATER)
      {
         ...
         REPA(visible_lakes ){Lake  *w=visible_lakes [i]; if(!w->removed)w->draw((sel_lake ==w) ? SelColor : (lit_lake ==w) ? LitColor : DefColor, (sel_lake ==w) ? ColorAlpha(SelColor, 0.08f) : TRANSPARENT);}
         REPA(visible_rivers){River *w=visible_rivers[i]; if(!w->removed)w->draw((sel_river==w) ? SelColor : (lit_river==w) ? LitColor : DefColor, (sel_river==w) ? ColorAlpha(SelColor, 0.08f) : TRANSPARENT);}
   ...
   {
      Color color=ColorAlpha(0.15f), color2=ColorAlpha(0.43f);
   ...
   void WorldView::drawHeightmapImport()
   {
      if(ImportTerrain.areaRectValid())
      {
         ...
         Box  box(Vec(r.min.x, h.x, r.min.y), Vec(r.max.x, h.y, r.max.y)); bool dw=D.depthWrite(false); box.draw(ColorAlpha(RED, 0.3f), true); D.depthWrite(dw); box.draw(ColorAlpha(RED, 0.5f));
   ...
   void WorldView::drawObjPoints()
   {
    ...
    //&& v4.sel()<0 // draw cursor only if multiple viewports visible
      ...
      if(mode()==OBJECT)
      {
         ...
         Color lit_sel=ColorAlpha(LitSelColor, 0.75f),
                   sel=ColorAlpha(   SelColor, 0.75f),
               lit    =ColorAlpha(   LitColor, 0.75f),
               normal =ColorAlpha(   DefColor, 0.75f);
         REPA(obj_visible)
         {
            Obj &obj=*obj_visible[i]; if(obj.icon && PosToScreen(obj.pos(), pos))
            {
               ...
               VI.imagePart(Rect_C(pos, 0.035f), tex);
            }
         }
         VI.end();
      }
   ...
   void WorldView::drawArea2D()
   {
      SetMatrix();
      grid.func(visibleArea(), AreaDraw2D);
   }
   void WorldView::higlight()
   {
      flt a=1+(   hm_mtrl_highlight-Time.appTime())*0.8f; if(a>0)Highlight(   hm_mtrl_img.screenRect(), a*0.75f);
          a=1+(water_mtrl_highlight-Time.appTime())*0.8f; if(a>0)Highlight(water_mtrl_img.screenRect(), a*0.75f);
   }
   bool WorldView::selected()C {return Mode()==MODE_WORLD;}
   Cursor& WorldView::curAll(int i)  {return InRange(i, cur_touch) ? cur_touch[i] : cur;}
   int     WorldView::curTotal(     )C {return 1+cur_touch.elms()                        ;}
   Cursor* WorldView::findCur(Touch *touch) {if(!touch)return &cur; REPA(cur_touch)if(touch==cur_touch[i].touch())return &cur_touch[i]; return null;}
   void WorldView::HeightNearestFunc(Cell<Area> &cell, HeightmapY &hy)
   {
   ...
   flt WorldView::hmHeight(C Vec  &pos, bool *found) {return hmHeight(pos.xz(), found);}
   flt WorldView::hmHeight(C Vec2 &xz , bool *found)
   {
   ...
   flt WorldView::hmHeightNearest(C Vec2 &xz)
   {
      HeightmapY hy;
      hy.xz  =xz;
   ...
   Vec WorldView::hmNormalAvg(C Matrix &matrix, C Box &box) {if(Area *area=getArea(worldToArea(matrix.pos)))return area->hmNormalAvg(matrix, box); return Vec(0, 1, 0);}
   bool WorldView::hmColor(C Vec  &pos, Vec &color) {return hmColor(pos.xz(), color);}
   bool WorldView::hmColor(C Vec2 &xz , Vec &color) {if(Area *area=getArea(worldToArea(xz)))return area->hmColor(xz, color); color=1; return false;}
   MaterialPtr WorldView::hmMtrl(C Vec  &pos, C MaterialPtr &cur_mtrl) {return hmMtrl(pos.xz(), cur_mtrl);}
   MaterialPtr WorldView::hmMtrl(C Vec2 &xz , C MaterialPtr &cur_mtrl) {if(Area *area=getArea(worldToArea(xz)))return area->hmMtrl(xz, cur_mtrl); return null;}
   bool WorldView::gridAlign()C {return grid_align        ;}
   bool WorldView::gridAlignRound()C {return grid_align_round  ;}
   flt  WorldView::gridAlignSize()C {return grid_align_size_xz;}
   flt  WorldView::gridAlignSizeY()C {return grid_align_size_y ;}
   RectI WorldView::visibleArea()C {return visible_area  ;}
   int   WorldView::visibleRadius()C {return visible_radius;}
   void  WorldView::visibleRadius(int r) {Clamp(r, 0, MaxVisibleRadius); visible_radius=r; REPAO(view_props).toGui();}
   int   WorldView::hmSelSize()C {return hm_sel_size   ;}
   void  WorldView::hmSelSize(int r) {Clamp(r, 0, MaxVisibleRadius); hm_sel_size=r; REPAO(hm_props).toGui();}
   bool WorldView::selectionCenter(Vec &center)
   {
      switch(mode())
      {
         case OBJECT:
   ...
   bool WorldView::selectionZoom(flt &dist)
   {
      flt size=-1;
      switch(mode())
      {
 ...
 C UID* WorldView::ID(C EditWaypoint *waypoint)C {return waypoints.dataToKey(waypoint);}
 C UID* WorldView::ID(C Lake         *lake    )C {return lakes    .dataToKey(lake    );}
 C UID* WorldView::ID(C River        *river   )C {return rivers   .dataToKey(river   );}
   ...
   void WorldView::createTerrain()
   {
      flt h=0.05f;
      mode.tab(HEIGHTMAP)+=hm_add_rem  .create(Rect_LU(mode.tab(HEIGHTMAP).rect().ld()+Vec2(-0.13f, -0.01f), 0.20f, 0.055f), "Del / Add").func(HmAddRem, T).focusable(false).desc("Remove existing heightmaps with LeftClick\nInsert new heightmaps with RightClick"); hm_add_rem.mode=BUTTON_TOGGLE;
      mode.tab(HEIGHTMAP)+=hm_mtrl_text.create(Vec2(hm_add_rem.rect().right()+Vec2(0.015f, 0)), "Material", &ts);
      mode.tab(HEIGHTMAP)+=hm_mtrl_img .create(Rect_LU(hm_add_rem.rect().ru()+Vec2(0.140f, 0), 0.055f, 0.055f)).desc("Material used when creating new heightmaps\nDrag and drop a material here"); hm_mtrl_img.alpha_mode=ALPHA_NONE;
      ...
      Rect hm_rect=AddProperties(hm_props, mode.tab(HEIGHTMAP), hm_mtrl_img.rect().ru()+Vec2(0.01f, 0), h, 0.12f, &ts); REPAO(hm_props).autoData(this);
      mode.tab(HEIGHTMAP)+=hm_import.create(Rect_LU(hm_rect.ru()+Vec2(h*1.5f, 0), 0.165f, 0.055f), "Import").func(HmImport, T).focusable(false);
   ...
   void WorldView::createObj()
   {
      ...
      show_obj_grid.tab(0)+=obj_grid.create().move(show_obj_grid.rect().rd()-Vec2(0, 0.01f)-obj_grid.rect().ru());
      mode.tab(OBJECT)+=obj_hm_align.create(Rect_RU(show_obj_grid.rect().lu(), 0.15f, obj_op.rect().h()), "Ground").focusable(false).desc(S+"Align objects to heightmap on movement\nKeyboard Shortcut: "+Kb.ctrlCmdName()+"+H"); obj_hm_align.mode=BUTTON_TOGGLE; obj_hm_align.set(true);

      obj_op.tab(OP_MOVE )+=obj_pos  .create(obj_op.tab(OP_MOVE ).rect().down()-Vec2(0, 0.01f));
      obj_op.tab(OP_ROT  )+=obj_rot  .create(obj_op.tab(OP_ROT  ).rect().down()-Vec2(0, 0.01f));
      obj_op.tab(OP_SCALE)+=obj_scale.create(obj_op.tab(OP_SCALE).rect().down()-Vec2(0, 0.01f));
      ...
      mode.tab(OBJECT)+=obj_params.create(Rect_LU(obj_menu  .rect().ru()+Vec2(h, 0), 0.19f, obj_op.rect().h()), 0, obj_params_t, Elms(obj_params_t)).set(0).hide();
      mode.tab(OBJECT)+=obj_list  .create(Rect_LU(obj_params.rect().ru(), 0.14f, obj_op.rect().h()), "List" ).focusable(false).desc(S+"Show object list\nKeyboard Shortcut: "+Kb.ctrlCmdName()+"+L"); obj_list.mode=BUTTON_TOGGLE;
      mode.tab(OBJECT)+=obj_paint .create(Rect_LU(obj_list  .rect().ru(), 0.15f, obj_op.rect().h()), "Paint").focusable(false).desc(S+"Show object painter\nKeyboard Shortcut: "+Kb.ctrlCmdName()+"+P\n\nPaint objects with RightClick\nRemove objects with LeftClick"); obj_paint.mode=BUTTON_TOGGLE;
      param_edit.create(obj_params.tab(0), false, true);

      Gui+=ObjList .create();
      Gui+=ObjPaint.create();
   }
   void WorldView::createWaypoint()
   {
      flt h=0.05f;
      mode.tab(WAYPOINT)+=waypoint_op.create(Rect_U(mode.tab(WAYPOINT).rect().down()-Vec2(0, 0.01f), 0.055f*7, 0.055f), 0, (cchar**)null, 7);
      ...
      waypoint_op.tab(WPO_MOVE)+=waypoint_pos.create(waypoint_op.tab(WPO_MOVE).rect().down()-Vec2(0, 0.01f));
      ...
      mode.tab(WAYPOINT)+=waypoint_list.create(Rect_LU(waypoint_menu.rect().ru()+Vec2(h, 0), 0.14f, waypoint_op.rect().h()), "List").focusable(false).desc(S+"Show waypoint list\nKeyboard Shortcut: "+Kb.ctrlCmdName()+"+L"); waypoint_list.mode=BUTTON_TOGGLE;
      ...
      Rect r=AddProperties(waypoint_props, waypoint_props_region, Vec2(0.01f, -0.01f), h, 0.50f, &ts); REPAO(waypoint_props).autoData(this).changed(WaypointChanged, WaypointPreChanged); p.button.resize(Vec2(h, 0));
      waypoint_props_region.rect(Rect_LU(waypoint_list.rect().ru()+Vec2(h, 0), r.size()+0.01f*2+Vec2(h, 0)));

      waypoint_global_props.New().create("Line Smooth", MEMBER(WorldView, waypoint_subdivide)).range(1, 16).mouseEditSpeed(3);
      AddProperties(waypoint_global_props, mode.tab(WAYPOINT), waypoint_op.rect().lu()+Vec2(-0.34f, 0), h, 0.12f, &ts); REPAO(waypoint_global_props).autoData(this);

      Gui+=WaypointList.create();
   }
   void WorldView::createWater()
   {
      flt h=0.05f;
      mode.tab(WATER)+=water_op.create(Rect_U(mode.tab(WATER).rect().down()-Vec2(0, 0.01f), 0.055f*10*1.1f, 0.055f), 0, (cchar**)null, 10).func(WaterOpChanged, T); // make width slightly larger because "+S" occupies lot of space
      ...
      mode.tab(WATER)+=water_mode.create(Rect_RU(water_op.rect().lu()-Vec2(0.05f, 0), 0.28f, 0.055f), 0, water_mode_t, Elms(water_mode_t)).valid(true).set(0);
   ...
   WorldView& WorldView::create()
   {
      ...
      T+=set_grid_level.create(Rect_LU(show_grid   .rect().ld(), h, h*0.64f), "SET").func(GridLevel, T).focusable(false).desc("Set world grid plane level\nThis affects cursor position (can be useful if operating on objects placed very high or very low).");
      ...
      T+=mode.create(Rect_LU(locate.rect().ru()+Vec2(h, 0), 0.63f, h), 0, mode_t, Elms(mode_t), true).func(ModeChanged, T, true); // set immediate func call so Brush visibility will be set instantly
      ...
      Rect view_props_r=AddProperties(view_props, T, mode.rect().ru()+Vec2(h, 0), h, 0.12f, &ts); REPAO(view_props).autoData(this);

      T+=play.create(Rect_LU(view_props_r.ru()+Vec2(h, 0), 0.12f, h), "Play").func(Play, T).focusable(false);
   ...
   void WorldView::selectedChanged()
   {
      setMenu();
      flushSettings ();
      flushWaypoints();
   ...
   void WorldView::skinChanged()
   {
      MtrlBrush.setColors();
   }
   void WorldView::camCenter(bool only_with_selection, bool zoom)
   {
      Vec center; flt dist;
   ...
   void WorldView::camCenter(bool zoom){camCenter(false, zoom);}
   void WorldView::flushSettings()
   {
   ...
   void WorldView::setChangedWaypoint(EditWaypoint *waypoint, bool refresh_gui) {if(C UID *waypoint_id=ID(waypoint)){changed_waypoints.include(waypoint); if(ver){ver->changedWaypoint(*waypoint_id);                                                                       if(refresh_gui && waypoint==sel_waypoint)waypoint_pos.toGui();}}}
   void WorldView::setChangedLake(Lake         *lake                           ) {if(C UID *    lake_id=ID(lake    )){changed_lakes    .include(lake    ); if(ver){ver->changedLake    (*    lake_id); lake ->setMesh(Proj); Proj.rebuildWater(lake, null , * lake_id, *ver); if(               lake    ==sel_lake    )        waterToGui();}}}
   void WorldView::setChangedRiver(River        *river                          ) {if(C UID *   river_id=ID(river   )){changed_rivers   .include(river   ); if(ver){ver->changedRiver   (*   river_id); river->setMesh(Proj); Proj.rebuildWater(null, river, *river_id, *ver); if(               river   ==sel_river   )        waterToGui();}}}
   void WorldView::selWaypoint(EditWaypoint *waypoint, C UID &point)
   {
      sel_waypoint=waypoint; sel_waypoint_point=point; waypoint_pos.toGui(); setVisibleWaypoints(); flushWaypoints(); WaypointList.setCur();
   }
   void WorldView::waterToGui() {REPAO(water_props).toGui();}
   void WorldView::setWaterVis()
   {
   ...
   void WorldView::activate(Elm *elm)
   {
      set(elm); if(T.elm){Mode.set(MODE_WORLD); HideBig();}
   }
   void WorldView::toggle(Elm *elm)
   {
      if(elm==T.elm && selected())elm=null;
      activate(elm);
   }
      void WorldView::IDName::set(C UID &id, C Str &name) {T.id=id; T.name=name;}
      int WorldView::IDName::CompareName(C IDName &a, C IDName &b) {return Compare(a.name, b.name);}
   void WorldView::enumChanged()
   {
   ...
   void WorldView::meshVariationChanged() {param_edit.meshVariationChanged();}
   void WorldView::elmChanged(C UID &elm_id)
   {
   ...
   void WorldView::erasing(C UID &elm_id)
   {
   ...
   void WorldView::objTransChanged()
   {
      obj_pos  .reset();
      obj_scale.reset();
      obj_rot  .reset();
   }
   void WorldView::objToGui()
   {
      ObjList.setChanged();
      objTransChanged();
      param_edit.toGui();
   ...
   void WorldView::resize()
{
   ...
   void WorldView::setHmMtrl(C UID &id)
   {
   ...
   void WorldView::setWaterMtrl(C UID &id)
   {
   ...
   void WorldView::setSelWaterMtrl(C UID &id)
   {
   ...
   bool WorldView::SameNameExcludeIndex(Str a, Str b)
   {
   ...
   void WorldView::drag(Memc<UID> &elms, GuiObj* &focus_obj, C Vec2 &screen_pos)
   {
   ...
   void WorldView::SetShader(Cell<Area> &cell, ptr user, int thread_index) {ThreadMayUseGPUData(); cell().setShader();}
          void WorldView::setShader(                                            ) {grid.mtFunc(WorkerThreads, valid_area, SetShader);}
   void WorldView::updateAligning()
   {
      if(mode()==OBJECT)
      {
   ...
   void WorldView::UpdateVisibility(Cell<Area> &cell, WorldView &world) {world.updateVisibility(cell);}
          void WorldView::updateVisibility(Cell<Area> &cell)
   {
      Area &area=cell();
   ...
   void WorldView::updateVisibility()
   {
   ...
   void WorldView::updateCamCenter()
   {
   ...
   void WorldView::update(C GuiPC &gpc)
{
   ...
   void WorldView::HmCreate(Cell<Area> &cell, Memt<Area*> &created) {if(cell().hmCreate(true))created.add(cell.data());}
   void WorldView::HmDel(Cell<Area> &cell, Memt<Area*> &created) {   cell().hmDel   (    );}
   void WorldView::HmBuild(     Area* &area, ptr user, int thread_index) {area->hmBuild();}
   void WorldView::highlightHmMtrl() {   hm_mtrl_highlight=Time.appTime();}
   void WorldView::highlightWaterMtrl() {water_mtrl_highlight=Time.appTime();}
   void WorldView::editHm(bool insert, Cursor &cur)
   {
   ...
   void WorldView::updateHm()
   {
      ...
      )
      {
         ...
         if(cur.valid() && !(ObjPaint.available() && !ObjPaint.ready())) // don't draw brush when ObjPaint is in get-material mode
         {
            ...
            SPSet("XZCol"         , Vec(0.33f, 0.66f, 1.00f)*0.45f);
         }else
         {
            use_shader=false;
            SPSet("XZRange", EPS);
      ...
      if((mode()==OBJECT && grid_align) || mode()==PATH)
      {
         ...
         Vec col(0.33f, 0.66f, 1);
         switch(mode())
         {
            case OBJECT: col*=(grid_align ? 0.3f : 0.1f); break;
            case PATH  : col*=0.2f; if(elm)if(ElmWorld *ew=elm->worldData())size=ew->cell_size; break;
         }
         SPSet("XZRange", size);
         SPSet("XZSoft" , 0.08f);
         SPSet("XZCol"  , col );
   ...
   void WorldView::ValidateRefs(Cell<Area> &cell, ptr) {cell().validateRefs();}
   void        WorldView::validateRefs() {grid.func(ValidateRefs); validate_refs_time=FLT_MAX;}
   void WorldView::delayedValidateRefs() {MIN(validate_refs_time, Time.appTime()+2);}
   void  WorldView::updateValidateRefs() {if(Time.appTime()>=validate_refs_time)validateRefs();}
   void WorldView::removed(Obj &obj) {REP(curTotal())curAll(i).removed(obj); obj_visible.binaryExclude(&obj, Obj::ComparePtr); obj_update.binaryExclude(&obj, Obj::ComparePtr);}
   void WorldView::hmDel(C VecI2 &area_xy, C TimeStamp *time)
   {
      if(elm)
      {
   ...
   void WorldView::setObjVisibility()
   {
   ...
   void WorldView::draw(C GuiPC &gpc)
{
      if(gpc.visible && visible())
      {
         ...
         if(a.elms())
         {
            flt x=rect().min.x+gpc.offset.x+0.01f, y=rect().max.y+gpc.offset.y-0.25f;
...
WorldView::HeightmapY::HeightmapY() : find(null) {}
