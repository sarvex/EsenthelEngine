   ...
   void LodView::draw(C GuiPC &gpc)
{
      if(gpc.visible && visible())
      {
         ...
         TextStyleParams ts; ts.size=0.039f;
   ...
   void LodRegion::Erase(LodView   &lod) {ObjEdit.  eraseLOD(lod.lod_index);}
   void LodRegion::Disable(LodView   &lod) {ObjEdit.disableLOD(lod.lod_index);}
   void LodRegion::Disable(LodRegion &lr ) {ObjEdit.disableLOD((ObjEdit.lit_lod>=0) ? ObjEdit.lit_lod :  ObjEdit.selLod());}
   ...
   void LodRegion::resize()
   {
      Vec2 old=edit_dist.posRU();
      edit_dist.rect(Rect_D(ObjEdit.rect().w()/2-0.16f, -ObjEdit.rect().h(), 0.32f, 0.055f));
      edit_dist_text.pos(edit_dist.rect().ru()+Vec2(0, ts.size.y/2+0.01f));
   ...
   void LodRegion::update(C GuiPC &gpc)
{
      ...
      if(gpc.visible && visible())
      {
         ...
         FREPA(lods)
         {
            ...
            lod.lod_index=i;
            Rect_LU r(i*s, 0, s, s); lod.rect(r); r.extend(-0.005f);
            lod.erase  .rect(Rect_RU(r.ru  (),      0.045f, 0.045f)).visible(ObjEdit.lit_lod==i && in_range /*&& ObjEdit.mesh.lods()>1 always display to just empty last LOD*/);
            lod.disable.rect(Rect_D (r.down(), r.w()*0.85f, 0.049f)).visible(ObjEdit.lit_lod==i && in_range);
            if(in_range)
            {
   ...
   void LodRegion::drop(Memc<Str> &names, GuiObj *focus_obj, C Vec2 &screen_pos)
   {
   ...
   void LodRegion::drag(Memc<UID> &elms, GuiObj* &focus_obj, C Vec2 &screen_pos)
   {
      if(ObjEdit.obj_elm && contains(focus_obj))REPAD(l, lods)if(lods[l].contains(focus_obj))
      {
         REPA(elms)
                     ...
                     if(ElmMesh *mesh_data=mesh_elm->meshData())
         {
            Mesh src; if(Load(src, Proj.editPath(mesh_elm->id), Proj.game_path))if(src.is())
            {
               ...
               src.setTanBinDbl().setRender(); // set tan/bin because src mesh from disk doesn't have them
   ...
   flt NewLodClass::NextLodDist(flt dist) {flt next=Max(2, Abs(dist)*2); if(NegativeSB(dist))CHSSB(next); return next;}
   void NewLodClass::FromFile(NewLodClass &nl)
   {
   ...
   void    NewLodClass::PreviewToggle(NewLodClass &nl) {if(nl.         preview_prop)nl.         preview_prop->set(!nl.         preview_prop->asBool());}
   void NewLodClass::DrawAtDistToggle(NewLodClass &nl) {if(nl.draw_at_distance_prop)nl.draw_at_distance_prop->set(!nl.draw_at_distance_prop->asBool());}
   void NewLodClass::OK(NewLodClass &nl)
   {
   ...
   void NewLodClass::ChangedParams(C Property &prop) {NewLod.changedParams();}
          void NewLodClass::changedParams()
   {
      stop=true;
      change_id++;
      finished=false;
      ...
      if(ObjEdit.getMeshElm()) // make sure mesh is created
      {
            ...
            if(temp.is())
         {
            ...
            FREPA(lod)
            {
               ...
               if(!base.vtx.tan() || !base.vtx.bin())base.setTanBinDbl(); //if(!base.vtx.tan())base.setTangents(); if(!base.vtx.bin())base.setBinormals(); // set in case mesh doesn't have them yet
   ...
   bool NewLodClass::needRebuild()C {return src_id!=simplified_src_id || change_id!=simplified_change_id;}
   bool NewLodClass::Simplify(Thread &thread) {return ((NewLodClass*)thread.user)->simplify();}
          bool NewLodClass::simplify()
   {
   ...
   void NewLodClass::startThread() {if(!thread.active())thread.create(Simplify, this);}
   void NewLodClass::createSrc()
   {
      {
...
draw_at_distance_prop=&props.New().create("Draw at Distance", MEMBER(NewLodClass, draw_at_distance)).desc("Draw LOD at specified distance\nKeyboard Shortcut: Alt+D");
   draw_distance_prop=&props.New().create("Draw Distance"   , MEMBER(NewLodClass, draw_distance   )).min(0).mouseEditMode(PROP_MOUSE_EDIT_SCALAR).mouseEditSpeed(0.5f);
                       props.New();
                       props.New().create("Intensity"       , MEMBER(NewLodClass, intensity       )).range(0,   1).mouseEditSpeed(0.4f).desc("How much to simplify\n0..1\n0=no simplification\n1=full simplification");
      ...
      ts.reset().size=0.045f; ts.align.set(1, 0); Rect r=AddProperties(props, T, Vec2(0.02f, -0.02f), 0.05f, 0.2f, &ts); REPAO(props).autoData(&NewLod).changed(ChangedParams); preview_prop->changed(null, null); draw_at_distance_prop->changed(null, null); draw_distance_prop->changed(null, null);
      T+=       ok.create(Rect_U(r.down()-Vec2(0, 0.05f), 0.3f, 0.06f), "OK").func(OK, T);
      T+=from_file.create(Rect_U(ok.rect().down()-Vec2(0, 0.01f), 0.3f, 0.06f), "From File").func(FromFile, T);
      r|=from_file.rect(); Vec2 size=Vec2(r.max.x, -r.min.y)+0.02f+defaultInnerPaddingSize();
...
NewLodClass::NewLodClass() : preview(true), simplified_valid(false), processed_ready(false), keep_border(false), draw_at_distance(false), finished(false), stop(false), intensity(0.5f), max_distance(1), max_uv(1), max_color(1), max_material(1), max_skin(1), max_normal(180), draw_distance(2), scale(1), src_id(0), simplified_src_id(0), change_id(0), simplified_change_id(0), quality(null), preview_prop(null), draw_at_distance_prop(null) {}
