...
void ObjView::meshAlign(bool xz)
{
   ...
   if(C MeshPart *part=getPart(lit_vf_part))if(InRange(lit_vtx, part->base.vtx))
   {
      Vec pos=part->base.vtx.pos(lit_vtx)*mesh_matrix,
         frac=Frac(pos);
      if(frac.x>0.5f)frac.x--;
      if(frac.y>0.5f)frac.y--;
      if(frac.z>0.5f)frac.z--;
      if(xz)frac.y=0;
...
void ObjView::meshNrmY()
{
   ...
   if(mesh_parts.edit_selected())
   {
      Memt<VecI2> vtxs; getSelectedVtxs(vtxs); if(vtxs.elms())
      {
         ...
         {
          C VecI2 &v=vtxs[i];
            if(MeshPart *part=lod.parts.addr(v.x))
...
void ObjView::meshRotQuads()
{
   ...
   REPA(sel_face)
   {
    C VecI2 &sf=sel_face[i]; if(sf.y&SIGN_BIT) // only quads
      {
         int q=sf.y^SIGN_BIT;
         if(MeshPart *part=lod.parts.addr(sf.x))if(InRange(q, part->base.quad))
         {
   ...
   if(mesh_parts.edit_selected())
   {
      ...
      REPA(sel_face)
      {
       C VecI2 &sf=sel_face[i]; if(sf.y&SIGN_BIT) // only quads
         {
            int q=sf.y^SIGN_BIT;
            if(MeshPart *part=lod.parts.addr(sf.x))if(InRange(q, part->base.quad))part_quads(sf.x).add(q);
         }
      }
      if(part_quads.elms())
      {
         REPA(part_quads)if(MeshPart *part=lod.parts.addr(i)){part->base.quadToTri(part_quads[i]); part->setRender();}
         changed=true;
      }
   }else
   {
...
void ObjView::meshTriToQuad()
{
   ...
   {
      REPA(lod)if(partOp(i)){MeshPart &part=lod.parts[i]; part.base.triToQuad(0.7f); part.setRender(); changed=true;}
   }
   if(changed)
   {
      litSelVFClear();
...
void ObjView::meshDisableLQLODs()
{
   mesh_undos.set("disableLQLODs");
   if(DisableLQLODs(mesh)){setChangedMesh(true, false); lod.toGui();}
}
