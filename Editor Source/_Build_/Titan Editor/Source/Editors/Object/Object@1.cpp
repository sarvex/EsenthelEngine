      ...
      void ObjView::PhysChange::apply(ptr user)
{
            ...
            if(ListElm *list_elm=Proj.list.visToData(Proj.list.lit))
               if(Elm *obj=list_elm->elm)if(ElmObj *obj_data=obj->objData())if(obj_data->mesh_id.valid())
                  if(C MeshPtr &mesh=Proj.gamePath(obj_data->mesh_id))
                     mesh->drawBlend(MatrixIdentity, &NoTemp(Vec4(1, 1, 1, 0.5f)));
          ...
          void ObjView::draw(Edit::Viewport4::View &view)
   {
      ...
      if(lit_vtx>=0 || lit_face!=-1)if(C MeshPart *part=getPart(lit_vf_part))if(C Vec *pos=part->base.vtx.pos())
      {
         ...
         if(lit.elms())
         {
            REPA(lit){int vtx=lit[i]; VI.dot(sel_vtx.binaryHas(VecI2(lit_vf_part, vtx)) ? SelColor : LitColor, pos[vtx], 0.005f);}
            VI.end();
         }
         if(InRange(lit_vtx, part->base.vtx))part->base.vtx.pos(lit_vtx).draw(sel_vtx.binaryHas(VecI2(lit_vf_part, lit_vtx)) ? SelColor : LitColor, 0.01f);
   ...
   void ObjView::Undo(ObjView &editor) {if(editor.mode()==PHYS)editor.phys_undos.undo();else if(editor.mode()==PARAM)editor.param_edit.undos.undo();else editor.mesh_undos.undo();}
   void ObjView::Redo(ObjView &editor) {if(editor.mode()==PHYS)editor.phys_undos.redo();else if(editor.mode()==PARAM)editor.param_edit.undos.redo();else editor.mesh_undos.redo();}
   ...
   void ObjView::MeshCopyNormal(ObjView &editor) {editor.meshCopyNrm    (         );}
   ...
   void ObjView::MeshVFS4(ObjView &editor) {editor.vtx_face_sel_mode.toggle(4);}
   void ObjView::MeshVFS5(ObjView &editor) {editor.vtx_face_sel_mode.toggle(5);}
   void ObjView::MeshVFS6(ObjView &editor) {editor.vtx_face_sel_mode.toggle(6);}
   ...
   void ObjView::MeshRemVtxTex0(ObjView &editor) {editor.remVtx(VTX_TEX0                  , true);}
   void ObjView::MeshRemVtxTex1(ObjView &editor) {editor.remVtx(VTX_TEX1                  , true);}
   void ObjView::MeshRemVtxTex2(ObjView &editor) {editor.remVtx(         VTX_TEX2         , true);}
   void ObjView::MeshRemVtxTex3(ObjView &editor) {editor.remVtx(                  VTX_TEX3, true);}
   void ObjView::MeshRemVtxTex123(ObjView &editor) {editor.remVtx(VTX_TEX1|VTX_TEX2|VTX_TEX3, true);}
 ...
 C MeshPart* ObjView::getPart(int i)C {return getLod().parts.addr(i);}
   ...
   void ObjView::getSkel(GuiObj *go, C Vec2 &screen_pos, int *bone_i, int *slot_i)
   {
      ...
      if(C Skeleton *skel=getVisSkel())if(Edit::Viewport4::View *view=v4.getView(go))
      {
         ...
         || slot_i && *slot_i>=0)
            if(dist>0.04f){if(bone_i)*bone_i=-1; if(slot_i)*slot_i=-1;}
      }
   }
   ...
   void ObjView::camCenter(bool zoom)
{
   ...
   void ObjView::animate(C AnimSkel &anim_skel, bool transform_anims, C UID &ignore_anim_id)
   {
            ...
            case SLOT_SCALE:
            {
               ...
               if(mesh_skel && InRange(sel_slot, mesh_skel->slots))if(MT.b(i, MT.mouse(i)))
               {
                  ...
                  switch(slot_tabs())
                  {
                     ...
                     {
                        Ms.freeze();
                        flt mul=(Kb.shift() ? 0.1f : 1.0f)*0.5f, angle=-MT.ad(i).sum()*mul;
                        switch(slot_axis)
                        {
...
cur_skel_to_saved_skel.removeBone(bone->name);
   ...
   void ObjView::SkelReplace(ObjView &editor ) {editor.skelReplace(false);}
   void ObjView::SkelReplaceByName(ObjView &editor ) {editor.skelReplace(true );}
          void ObjView::skelReplace(bool     by_name)
   {
   ...
   void ObjView::BoneSetShape(ObjView &editor) {editor.boneSetShape();}
          void ObjView::boneSetShape()
   {
      if(mesh_skel)
      {
...
cur_skel_to_saved_skel.renameBone(old_name, new_name);
   ...
   void ObjView::NewBox(ObjView &editor) {if(editor.getPhysElm() && editor.phys){editor.phys_undos.set("box"    ); if(editor.hasPhysMeshOrConvex())editor.physDel(); MeshBase m; editor.physMesh(m); Box  box =m;                      {CacheLock cl(PhysBodies); editor.phys->parts.New().create(OBox(Box(Max(box.w(), 0.01f), Max(box.h(), 0.01f), Max(box.d(), 0.01f), box.center()))); editor.phys->setBox();} editor.setChangedPhys();}}
   ...
   void ObjView::NewCapsule(ObjView &editor) {if(editor.getPhysElm() && editor.phys){editor.phys_undos.set("capsule"); if(editor.hasPhysMeshOrConvex())editor.physDel(); MeshBase m; editor.physMesh(m); Box  box =m;                      {CacheLock cl(PhysBodies); editor.phys->parts.New().create(Capsule(Max(box.xz().size().max()/2, 0.005f), Max(box.h(), 0.01f), box.center())     ); editor.phys->setBox();} editor.setChangedPhys();}}
   void ObjView::NewTube(ObjView &editor) {if(editor.getPhysElm() && editor.phys){editor.phys_undos.set("tube"   ); if(editor.hasPhysMeshOrConvex())editor.physDel(); MeshBase m; editor.physMesh(m); Box  box =m;                      {CacheLock cl(PhysBodies); editor.phys->parts.New().create(Tube   (Max(box.xz().size().max()/2, 0.005f), Max(box.h(), 0.01f), box.center())     ); editor.phys->setBox();} editor.setChangedPhys();}}
