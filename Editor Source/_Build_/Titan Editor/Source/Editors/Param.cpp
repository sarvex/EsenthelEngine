      ...
      void ParamEditor::Change::apply(ptr user)
{
   ...
   void ParamEditor::create(GuiObj &parent, bool is_class, bool world)
   {
      ...
      if(!is_class)
      {
         ...
         T+=t_path    .create(Vec2(0.01f, y), "Path Mesh", &ts_white);          T+=o_path    .create(Rect_L(0.17f, y, h, h)).func(ChangedOPath         , T).desc("Override default path mesh"     );                                                                           T+=v_path    .create(Rect_L(0.22f          , y, 0.4f, h)).func(ChangedPath         , T).desc("Object Path Mesh Generation Mode."); v_path.setColumns(NameDescListColumn, Elms(NameDescListColumn)).setData(path_mode, Elms(path_mode)).menu.list.setElmDesc(MEMBER(NameDesc, desc)); y-=h;
      ...
      if(is_class)
      {
         ...
         param_window+=t_path       .create(Vec2(0.01f, y), "Path Mesh", &ts_black); param_window+=v_path       .create(Rect_L(0.18f           , y, 0.23f, h)).func(ChangedPath      , T).desc("Object Path Mesh Generation Mode."            ); v_path       .setColumns(NameDescListColumn, Elms(NameDescListColumn)).setData( path_mode, Elms( path_mode)).menu.list.setElmDesc(MEMBER(NameDesc, desc)); y-=h;
