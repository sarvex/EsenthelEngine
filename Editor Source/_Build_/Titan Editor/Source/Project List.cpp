...
/******************************************************************************/
   ...
   void Projects::ImportProj(Projects &projs) {if(!projs.import_proj_io.is())projs.import_proj_io.create(ProjectPackageExt, S, S, ImportProj); projs.import_proj_io.activate();}
   ...
   void Projects::resize()
   {
      ...
      proj_region    .rect(Rect(-D.w()+0.05f, -D.h()+0.06f, D.w()-0.05f, t_login_desc.rect().min.y-t_login_desc.textSize().y-0.08f));
   ...
   void RenameProjWin::update(C GuiPC &gpc)
{
   ...
   void RemoveProjWin::create()
   {
      ...
      T  +=t_name.create(Rect_C(clientWidth()/2, -0.11f, 0.7f, 0.0f), "Are you sure you wish to delete selected project from your disk?"); t_name.auto_line=true;
      T  +=    ok.create(Rect_C(clientWidth()/2, -0.23f, 0.3f, 0.06f), "OK").func(OK, T);
   }
